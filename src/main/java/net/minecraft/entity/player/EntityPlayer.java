...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void onUpdate()
    {
        ...
        double doubleValue = this.posX - this.chasingPosX;
        double secondDoubleValue = this.posY - this.chasingPosY;
        double thirdDoubleValue = this.posZ - this.chasingPosZ;
        double fourthDoubleValue = 10.0D;

        if (doubleValue > fourthDoubleValue)
        {
            this.prevChasingPosX = this.chasingPosX = this.posX;
        }

        if (thirdDoubleValue > fourthDoubleValue)
        {
            this.prevChasingPosZ = this.chasingPosZ = this.posZ;
        }

        if (secondDoubleValue > fourthDoubleValue)
        {
        ...
        this.chasingPosX += doubleValue * 0.25D;
        this.chasingPosZ += thirdDoubleValue * 0.25D;
        this.chasingPosY += secondDoubleValue * 0.25D;
        ...
        int i = 29999999;
        double fifthDoubleValue = MathHelper.clamp_double(this.posX, -2.9999999E7D, 2.9999999E7D);
        double eighthDoubleValue = MathHelper.clamp_double(this.posZ, -2.9999999E7D, 2.9999999E7D);

        if (fifthDoubleValue != this.posX || eighthDoubleValue != this.posZ)
        {
            this.setPosition(fifthDoubleValue, this.posY, eighthDoubleValue);
        }
    }
    ...
    protected void updateItemUse(ItemStack itemStackIn, int particleCount)
    {
        ...
        if (itemStackIn.getItemUseAction() == EnumAction.EAT)
        {
            for (int i = 0; i < particleCount; ++i)
            {
                ...
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, secondLocalValue.xCoord, secondLocalValue.yCoord, secondLocalValue.zCoord, localValue.xCoord, localValue.yCoord + 0.05D, localValue.zCoord, new int[] {Item.getIdFromItem(itemStackIn.getItem()), itemStackIn.getMetadata()});
                }
                else
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, secondLocalValue.xCoord, secondLocalValue.yCoord, secondLocalValue.zCoord, localValue.xCoord, localValue.yCoord + 0.05D, localValue.zCoord, new int[] {Item.getIdFromItem(itemStackIn.getItem())});
                }
            }
    ...
    public void updateRidden()
    {
        ...
        else
        {
            ...
            float floatValue2 = this.rotationPitch;
            super.updateRidden();
            this.prevCameraYaw = this.cameraYaw;
            this.cameraYaw = 0.0F;
            this.addMountedMovementStat(this.posX - xCoordinate, this.posY - yCoordinate, this.posZ - zCoordinate);
    ...
    public void onLivingUpdate()
    {
        ...
        float floatValue2 = (float)(Math.atan(-this.motionY * 0.20000000298023224D) * 15.0D);
        ...
        this.cameraPitch += (floatValue2 - this.cameraPitch) * 0.8F;
    ...
    {
        this.dataWatcher.updateObject(18, Integer.valueOf(score));
    }

    public void addScore(int score)
    {
        int i = this.getScore();
        this.dataWatcher.updateObject(18, Integer.valueOf(i + score));
    }
    ...
    public void addToPlayerScore(Entity entityIn, int amount)
    {
        ...
        if (entityIn instanceof EntityPlayer)
        {
            ...
            collection.addAll(this.getTeamKillObjectives(entityIn));
        }
        else
        {
    ...
    private Collection<ScoreObjective> getTeamKillObjectives(Entity entityIn)
    {
        ...
        if (scoreplayerteam != null)
        {
            ...
            if (i >= 0 && i < IScoreObjectiveCriteria.KILLED_BY_TEAM.length)
            {
                ...
                {
                    Score score = this.getWorldScoreboard().getValueFromObjective(entityIn.getName(), scoreobjective);
                    score.incrementScore();
                }
            }
        }

        ScorePlayerTeam scoreplayerteam1 = this.getWorldScoreboard().getPlayersTeam(entityIn.getName());
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        {
            double doubleValue = this.posY - 0.30000001192092896D + (double)this.getEyeHeight();
            EntityItem entityitem = new EntityItem(this.worldObj, this.posX, doubleValue, this.posZ, droppedItem);
            ...
            if (dropAround)
            {
                ...
                float floatValue = this.rand.nextFloat() * (float)Math.PI * 2.0F;
                entityitem.motionX = (double)(-MathHelper.sin(floatValue) * f);
                entityitem.motionZ = (double)(MathHelper.cos(floatValue) * f);
            ...
            else
            {
                ...
                float secondFloatValue = this.rand.nextFloat() * (float)Math.PI * 2.0F;
                thirdFloatValue = 0.02F * this.rand.nextFloat();
                entityitem.motionX += Math.cos((double)secondFloatValue) * (double)thirdFloatValue;
                entityitem.motionY += (double)((this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F);
                entityitem.motionZ += Math.sin((double)secondFloatValue) * (double)thirdFloatValue;
            }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity)
    {
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                {
                    floatValue = EnchantmentHelper.getModifierForCreature(this.getHeldItem(), ((EntityLivingBase)targetEntity).getCreatureAttribute());
                }
                else
                {
                    floatValue = EnchantmentHelper.getModifierForCreature(this.getHeldItem(), EnumCreatureAttribute.UNDEFINED);
                }
                ...
                if (f > 0.0F || floatValue > 0.0F)
                {
                    ...
                    double sixthDoubleValue = targetEntity.motionX;
                    double doubleValue = targetEntity.motionY;
                    double secondDoubleValue = targetEntity.motionZ;
                    ...
                    if (flag2)
                    {
                        ...
                        if (targetEntity instanceof EntityPlayerMP && targetEntity.velocityChanged)
                        {
                            ...
                            targetEntity.motionX = sixthDoubleValue;
                            targetEntity.motionY = doubleValue;
                            targetEntity.motionZ = secondDoubleValue;
                        }
    ...
    public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation)
    {
        if (!this.worldObj.isRemote)
        {
            ...
            double seventhDoubleValue = 8.0D;
            double doubleValue2 = 5.0D;
            List<EntityMob> list = this.worldObj.<EntityMob>getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB((double)bedLocation.getX() - seventhDoubleValue, (double)bedLocation.getY() - doubleValue2, (double)bedLocation.getZ() - seventhDoubleValue, (double)bedLocation.getX() + seventhDoubleValue, (double)bedLocation.getY() + doubleValue2, (double)bedLocation.getZ() + seventhDoubleValue));
        ...
        if (this.worldObj.isBlockLoaded(bedLocation))
        {
            ...
            this.setPosition((double)((float)bedLocation.getX() + f), (double)((float)bedLocation.getY() + 0.6875F), (double)((float)bedLocation.getZ() + floatValue2));
        }
        else
        {
    ...
    public void moveEntityWithHeading(float strafe, float forward)
    {
        ...
        this.addMovementStat(this.posX - doubleValue, this.posY - secondDoubleValue, this.posZ - thirdDoubleValue);
    }
    ...
    public void addMovementStat(double distanceX, double distanceY, double distanceZ)
    {
        if (this.ridingEntity == null)
        {
            if (this.isInsideOfMaterial(Material.water))
            {
                int i = Math.round(MathHelper.sqrt_double(distanceX * distanceX + distanceY * distanceY + distanceZ * distanceZ) * 100.0F);
            ...
            {
                int j = Math.round(MathHelper.sqrt_double(distanceX * distanceX + distanceZ * distanceZ) * 100.0F);
            ...
            else if (this.isOnLadder())
            {
                if (distanceY > 0.0D)
                {
                    this.addStat(StatList.distanceClimbedStat, (int)Math.round(distanceY * 100.0D));
                }
            }
            else if (this.onGround)
            {
                int k = Math.round(MathHelper.sqrt_double(distanceX * distanceX + distanceZ * distanceZ) * 100.0F);
            ...
            {
                int l = Math.round(MathHelper.sqrt_double(distanceX * distanceX + distanceZ * distanceZ) * 100.0F);
    ...
    private void addMountedMovementStat(double distanceX, double distanceY, double distanceZ)
    {
        if (this.ridingEntity != null)
        {
            int i = Math.round(MathHelper.sqrt_double(distanceX * distanceX + distanceY * distanceY + distanceZ * distanceZ) * 100.0F);
    ...
    public IChatComponent getDisplayName()
    {
        ...
        this.displayNameCache = ichatcomponent;
    ...
    {
        return (this.getDataWatcher().getWatchableObjectByte(10) & modelPart.getPartMask()) == modelPart.getPartMask();
    }
    ...
    public static enum EnumChatVisibility
    {
        ...
        public static final EntityPlayer.EnumChatVisibility[] VALUES = values();
