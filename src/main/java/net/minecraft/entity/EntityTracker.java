...
public class EntityTracker
{
    ...
    public void updateVisibility(EntityPlayerMP playerMP)
    {
        for (EntityTrackerEntry entityTrackerEntry : this.trackedEntities)
        {
            if (entityTrackerEntry.trackedEntity == playerMP)
            {
                entityTrackerEntry.updatePlayerEntities(this.theWorld.playerEntities);
            }
            else
            {
                entityTrackerEntry.updatePlayerEntity(playerMP);
            }
        }
    }
    ...
    public void sendToTrackingAndSelf(Entity entityIn, Packet packetIn)
    {
        ...
        if (entitytrackerentry != null)
        {
            entitytrackerentry.sendPacketToTrackedPlayersAndTrackedEntity(packetIn);
        }
    }
    ...
    public void sendEntitiesInChunkToPlayer(EntityPlayerMP playerMP, Chunk chunkIn)
    {
        for (EntityTrackerEntry entitytrackerentry : this.trackedEntities)
        {
            if (entitytrackerentry.trackedEntity != playerMP && entitytrackerentry.trackedEntity.chunkCoordX == chunkIn.xPosition && entitytrackerentry.trackedEntity.chunkCoordZ == chunkIn.zPosition)
            {
                entitytrackerentry.updatePlayerEntity(playerMP);
            }
        }
