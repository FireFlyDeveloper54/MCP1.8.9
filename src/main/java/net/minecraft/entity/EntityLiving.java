...
public abstract class EntityLiving extends EntityLivingBase
{
    ...
    public void spawnExplosionParticle()
    {
        if (this.worldObj.isRemote)
        {
            for (int i = 0; i < 20; ++i)
            {
                double fifthDoubleValue = this.rand.nextGaussian() * 0.02D;
                double sixthDoubleValue = this.rand.nextGaussian() * 0.02D;
                double seventhDoubleValue = this.rand.nextGaussian() * 0.02D;
                double eighthDoubleValue = 10.0D;
                this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width - fifthDoubleValue * eighthDoubleValue, this.posY + (double)(this.rand.nextFloat() * this.height) - sixthDoubleValue * eighthDoubleValue, this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width - seventhDoubleValue * eighthDoubleValue, fifthDoubleValue, sixthDoubleValue, seventhDoubleValue, EnumParticleTypes.EMPTY_ARGS);
            }
        }
        else
        {
    ...
    protected void despawnEntity()
    {
        ...
        else
        {
            ...
            if (entity != null)
            {
                double doubleValue = entity.posX - this.posX;
                double secondDoubleValue = entity.posY - this.posY;
                double thirdDoubleValue = entity.posZ - this.posZ;
                double fourthDoubleValue = doubleValue * doubleValue + secondDoubleValue * secondDoubleValue + thirdDoubleValue * thirdDoubleValue;

                if (this.canDespawn() && fourthDoubleValue > 16384.0D)
                {
                    this.setDead();
                }

                if (this.entityAge > 600 && this.rand.nextInt(800) == 0 && fourthDoubleValue > 1024.0D && this.canDespawn())
                {
                    this.setDead();
                }
                else if (fourthDoubleValue < 1024.0D)
                {
                    this.entityAge = 0;
                }
            }
    ...
    public void faceEntity(Entity entityIn, float maxYawIncrease, float maxPitchIncrease)
    {
        ...
        if (entityIn instanceof EntityLivingBase)
        {
            ...
            doubleValue = entityLivingBase.posY + (double)entityLivingBase.getEyeHeight() - (this.posY + (double)this.getEyeHeight());
        }
        else
        {
            doubleValue = (entityIn.getEntityBoundingBox().minY + entityIn.getEntityBoundingBox().maxY) / 2.0D - (this.posY + (double)this.getEyeHeight());
        }

        double ninthDoubleValue = (double)MathHelper.sqrt_double(xCoordinate * xCoordinate + zCoordinate * zCoordinate);
        float f = (float)(MathHelper.atan2(zCoordinate, xCoordinate) * 180.0D / Math.PI) - 90.0F;
        float floatValue2 = (float)(-(MathHelper.atan2(doubleValue, ninthDoubleValue) * 180.0D / Math.PI));
        this.rotationPitch = this.updateRotation(this.rotationPitch, floatValue2, maxPitchIncrease);
        this.rotationYaw = this.updateRotation(this.rotationYaw, f, maxYawIncrease);
    }

    private float updateRotation(float currentAngle, float targetAngle, float maxIncrease)
    {
