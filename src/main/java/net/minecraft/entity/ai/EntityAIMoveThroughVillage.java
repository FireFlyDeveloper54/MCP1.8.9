...
public class EntityAIMoveThroughVillage extends EntityAIBase
{
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    boolean oldEnterDoors = pathNavigateGround.getEnterDoors();
                    pathNavigateGround.setBreakDoors(false);
                    this.entityPathNavigate = pathNavigateGround.getPathToPos(this.doorInfo.getDoorBlockPos());
                    pathNavigateGround.setBreakDoors(oldEnterDoors);
                    ...
                    {
                        Vec3 targetPos = RandomPositionGenerator.findRandomTargetBlockTowards(this.theEntity, 10, 7, new Vec3((double)this.doorInfo.getDoorBlockPos().getX(), (double)this.doorInfo.getDoorBlockPos().getY(), (double)this.doorInfo.getDoorBlockPos().getZ()));
                        ...
                        {
                            pathNavigateGround.setBreakDoors(false);
                            this.entityPathNavigate = this.theEntity.getNavigator().getPathToXYZ(targetPos.xCoord, targetPos.yCoord, targetPos.zCoord);
                            pathNavigateGround.setBreakDoors(oldEnterDoors);
    ...
    public boolean continueExecuting()
    {
        ...
        {
            float doorReachDistance = this.theEntity.width + 4.0F;
            return this.theEntity.getDistanceSq(this.doorInfo.getDoorBlockPos()) > (double)(doorReachDistance * doorReachDistance);
        }
    }
    ...
    private VillageDoorInfo findNearestDoor(Village villageIn)
    {
        ...
        for (VillageDoorInfo candidateDoor : villageIn.getVillageDoorInfoList())
        {
            int candidateDistanceSq = candidateDoor.getDistanceSquared(MathHelper.floor_double(this.theEntity.posX), MathHelper.floor_double(this.theEntity.posY), MathHelper.floor_double(this.theEntity.posZ));

            if (candidateDistanceSq < nearestDistanceSq && !this.doesDoorListContain(candidateDoor))
            {
                villageDoorInfo = candidateDoor;
