...
public class EntityHorse extends EntityAnimal implements IInvBasic
{
    ...
    private boolean hasValidTextureData = false;
    ...
    private boolean getHorseWatchableBoolean(int flag)
    {
        return (this.dataWatcher.getWatchableObjectInt(16) & flag) != 0;
    }

    private void setHorseWatchableBoolean(int flag, boolean value)
    {
        int i = this.dataWatcher.getWatchableObjectInt(16);

        if (value)
        {
            this.dataWatcher.updateObject(16, Integer.valueOf(i | flag));
        }
        else
        {
            this.dataWatcher.updateObject(16, Integer.valueOf(i & ~flag));
    ...
    protected void updateLeashedState(float leashDistance)
    {
        if (leashDistance > 6.0F && this.isEatingHaystack())
        {
    ...
    public int increaseTemper(int amount)
    {
        int i = MathHelper.clamp_int(this.getTemper() + amount, 0, this.getMaxTemper());
        this.setTemper(i);
        return i;
    }
    ...
    public void fall(float distance, float damageMultiplier)
    {
        ...
        if (i > 0)
        {
            ...
            Block block = this.worldObj.getBlockState(this.blockSamplePos.set(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY - 0.2D - (double)this.prevRotationYaw), MathHelper.floor_double(this.posZ))).getBlock();
    ...
    protected EntityHorse getClosestHorse(Entity entityIn, double distance)
    {
        ...
        {
            double xCoordinate = entity1.getDistanceSq(entityIn.posX, entityIn.posY, entityIn.posZ);

            if (xCoordinate < doubleValue)
            {
                entity = entity1;
    ...
    public boolean usesLayeredTexture()
    {
    ...
    {
        return this.hasValidTextureData;
    }
    ...
    private void setHorseTexturePaths()
    {
        ...
        int intValue = this.getHorseArmorIndexSynced();

        if (intValue >= horseArmorTextures.length)
        {
            this.hasValidTextureData = false;
        }
        else
        {
            this.horseTexturesArray[2] = horseArmorTextures[intValue];
            this.texturePrefix = this.texturePrefix + HORSE_ARMOR_TEXTURES_ABBR[intValue];
            this.hasValidTextureData = true;
        }
    }
    ...
    public boolean interact(EntityPlayer player)
    {
        ...
        else if (this.canBeRidden() && this.riddenByEntity != null)
        {
            return super.interact(player);
        }
        else
        {
            if (itemStack != null)
            {
                ...
                if (!flag && this.canBeRidden() && !this.isHorseSaddled() && itemStack.getItem() == Items.saddle)
                {
                    this.openGUI(player);
                    return true;
                }
            ...
            if (this.canBeRidden() && this.riddenByEntity == null)
            {
                if (itemStack != null && itemStack.interactWithEntity(player, this))
                {
                    return true;
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (!this.isEatingHaystack() && this.riddenByEntity == null && this.rand.nextInt(300) == 0 && this.worldObj.getBlockState(this.blockSamplePos.set(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ))).getBlock() == Blocks.grass)
            {
        ...
        if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityLivingBase && this.isHorseSaddled())
        {
            ...
            if (this.jumpPower > 0.0F && !this.isHorseJumping() && this.onGround)
            {
                ...
                if (forward > 0.0F)
                {
                    ...
                    float floatValue2 = MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F);
                    this.motionX += (double)(-0.4F * f * this.jumpPower);
                    this.motionZ += (double)(0.4F * floatValue2 * this.jumpPower);
    ...
    public EntityAgeable createChild(EntityAgeable ageable)
    {
        ...
        if (k == 0)
        {
            int intValue2 = this.rand.nextInt(9);
            int l;
            ...
            int secondIntValue = this.rand.nextInt(5);

            if (secondIntValue < 2)
            {
                l = l | this.getHorseVariant() & 65280;
            }
            else if (secondIntValue < 4)
            {
        ...
        double doubleValue = this.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue() + ageable.getEntityAttribute(SharedMonsterAttributes.maxHealth).getBaseValue() + (double)this.getModifiedMaxHealth();
        entityhorse1.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(doubleValue / 3.0D);
        double doubleValue2 = this.getEntityAttribute(horseJumpStrength).getBaseValue() + ageable.getEntityAttribute(horseJumpStrength).getBaseValue() + this.getModifiedJumpStrength();
        entityhorse1.getEntityAttribute(horseJumpStrength).setBaseValue(doubleValue2 / 3.0D);
        double doubleValue3 = this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getBaseValue() + ageable.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getBaseValue() + this.getModifiedMovementSpeed();
        entityhorse1.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(doubleValue3 / 3.0D);
        return entityhorse1;
    }
    ...
    {
        return this.prevHeadLean + (this.headLean - this.prevHeadLean) * partialTicks;
    }

    public float getRearingAmount(float partialTicks)
    {
        return this.prevRearingAmount + (this.rearingAmount - this.prevRearingAmount) * partialTicks;
    }

    public float getMouthOpennessAngle(float partialTicks)
    {
        return this.prevMouthOpenness + (this.mouthOpenness - this.prevMouthOpenness) * partialTicks;
    }
    ...
    protected void spawnHorseParticles(boolean spawnHearts)
    {
        EnumParticleTypes enumparticletypes = spawnHearts ? EnumParticleTypes.HEART : EnumParticleTypes.SMOKE_NORMAL;
    ...
    public void updateRiderPosition()
    {
        ...
        if (this.prevRearingAmount > 0.0F)
        {
            ...
            float floatValue2 = MathHelper.cos(this.renderYawOffset * (float)Math.PI / 180.0F);
            float floatValue3 = 0.7F * this.prevRearingAmount;
            float floatValue4 = 0.15F * this.prevRearingAmount;
            this.riddenByEntity.setPosition(this.posX + (double)(floatValue3 * f), this.posY + this.getMountedYOffset() + this.riddenByEntity.getYOffset() + (double)floatValue4, this.posZ - (double)(floatValue3 * floatValue2));
    ...
    public static boolean isArmorItem(Item itemIn)
    {
        return itemIn == Items.iron_horse_armor || itemIn == Items.golden_horse_armor || itemIn == Items.diamond_horse_armor;
    }
