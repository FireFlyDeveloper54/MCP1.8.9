...
public abstract class EntityCreature extends EntityLiving
{
    ...
    private final BlockPos.MutableBlockPos pathWeightSamplePos = new BlockPos.MutableBlockPos();
    ...
    {
        return super.getCanSpawnHere() && this.getBlockPathWeight(this.pathWeightSamplePos.set(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.getEntityBoundingBox().minY), MathHelper.floor_double(this.posZ))) >= 0.0F;
    }
    ...
    {
        return this.isWithinHomeDistanceFromPosition(this.pathWeightSamplePos.set(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)));
    }
    ...
    protected void updateLeashedState()
    {
        ...
        if (this.getLeashed() && this.getLeashedToEntity() != null && this.getLeashedToEntity().worldObj == this.worldObj)
        {
            ...
            if (f > 6.0F)
            {
                double xCoordinate = (entity.posX - this.posX) / (double)f;
                double yCoordinate = (entity.posY - this.posY) / (double)f;
                double zCoordinate = (entity.posZ - this.posZ) / (double)f;
                this.motionX += xCoordinate * Math.abs(xCoordinate) * 0.4D;
                this.motionY += yCoordinate * Math.abs(yCoordinate) * 0.4D;
                this.motionZ += zCoordinate * Math.abs(zCoordinate) * 0.4D;
            }
