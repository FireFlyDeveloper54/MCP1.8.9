...
public class EntitySlime extends EntityLiving implements IMob
{
    ...
    private final BlockPos.MutableBlockPos spawnSamplePos = new BlockPos.MutableBlockPos();
    ...
    public void onUpdate()
    {
        ...
        if (this.onGround && !this.wasOnGround)
        {
            ...
            for (int j = 0; j < i * 8; ++j)
            {
                ...
                float floatValue2 = this.rand.nextFloat() * 0.5F + 0.5F;
                float floatValue3 = MathHelper.sin(f) * (float)i * 0.5F * floatValue2;
                float floatValue4 = MathHelper.cos(f) * (float)i * 0.5F * floatValue2;
                World world = this.worldObj;
                EnumParticleTypes enumparticletypes = this.getParticleType();
                double xCoordinate = this.posX + (double)floatValue3;
                double zCoordinate = this.posZ + (double)floatValue4;
                world.spawnParticle(enumparticletypes, xCoordinate, this.getEntityBoundingBox().minY, zCoordinate, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
            }
    ...
    public void setDead()
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                float floatValue2 = ((float)(k / 2) - 0.5F) * (float)i / 4.0F;
                ...
                entitySlime.setLocationAndAngles(this.posX + (double)f, this.posY + 0.5D, this.posZ + (double)floatValue2, this.rand.nextFloat() * 360.0F, 0.0F);
    ...
    protected void dealDamage(EntityLivingBase target)
    {
        int i = this.getSlimeSize();

        if (this.canEntityBeSeen(target) && this.getDistanceSqToEntity(target) < 0.6D * (double)i * 0.6D * (double)i && target.attackEntityFrom(DamageSource.causeMobDamage(this), (float)this.getAttackStrength()))
        {
    ...
    {
        BlockPos blockPos = this.spawnSamplePos.set(MathHelper.floor_double(this.posX), 0, MathHelper.floor_double(this.posZ));
        ...
        else
        {
            if (this.worldObj.getDifficulty() != EnumDifficulty.PEACEFUL)
            {
                ...
                if (biomeGenBase == BiomeGenBase.swampland && this.posY > 50.0D && this.posY < 70.0D && this.rand.nextFloat() < 0.5F && this.rand.nextFloat() < this.worldObj.getCurrentMoonPhaseFactor() && this.worldObj.getLightFromNeighbors(this.spawnSamplePos.set(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ))) <= this.rand.nextInt(8))
                {
    ...
    static class SlimeMoveHelper extends EntityMoveHelper
    {
        ...
        public void setDirection(float yRotIn, boolean aggressiveIn)
        {
            this.yRot = yRotIn;
            this.aggressive = aggressiveIn;
        }
