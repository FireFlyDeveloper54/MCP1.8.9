...
public class EntityWitch extends EntityMob implements IRangedAttackMob
{
    ...
    public void handleStatusUpdate(byte id)
    {
        if (id == 15)
        {
            ...
            {
                this.worldObj.spawnParticle(EnumParticleTypes.SPELL_WITCH, this.posX + this.rand.nextGaussian() * 0.12999999523162842D, this.getEntityBoundingBox().maxY + 0.5D + this.rand.nextGaussian() * 0.12999999523162842D, this.posZ + this.rand.nextGaussian() * 0.12999999523162842D, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
            }
        }
        else
        {
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor)
    {
        if (!this.getAggressive())
        {
            ...
            double xCoordinate = target.posX + target.motionX - this.posX;
            double yCoordinate2 = yCoordinate - this.posY;
            double zCoordinate = target.posZ + target.motionZ - this.posZ;
            float f = MathHelper.sqrt_double(xCoordinate * xCoordinate + zCoordinate * zCoordinate);
            ...
            entityPotion.setThrowableHeading(xCoordinate, yCoordinate2 + (double)(f * 0.2F), zCoordinate, 0.75F, 8.0F);
