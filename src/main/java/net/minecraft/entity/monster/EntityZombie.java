...
public class EntityZombie extends EntityMob
{
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        if (super.attackEntityFrom(source, amount))
        {
            ...
            if (entitylivingbase != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD && (double)this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue())
            {
                ...
                for (int l = 0; l < 50; ++l)
                {
                    int intValue = i + MathHelper.getRandomIntegerInRange(this.rand, 7, 40) * MathHelper.getRandomIntegerInRange(this.rand, -1, 1);
                    int secondIntValue = j + MathHelper.getRandomIntegerInRange(this.rand, 7, 40) * MathHelper.getRandomIntegerInRange(this.rand, -1, 1);
                    int thirdIntValue = k + MathHelper.getRandomIntegerInRange(this.rand, 7, 40) * MathHelper.getRandomIntegerInRange(this.rand, -1, 1);
                    BlockPos supportPos = new BlockPos(intValue, secondIntValue - 1, thirdIntValue);

                    if (World.doesBlockHaveSolidTopSurface(this.worldObj, supportPos))
                    {
                        BlockPos spawnPos = new BlockPos(intValue, secondIntValue, thirdIntValue);

                        if (this.worldObj.getLightFromNeighbors(spawnPos) < 10)
                        {
                            entityzombie.setPosition((double)intValue, (double)secondIntValue, (double)thirdIntValue);

                            if (!this.worldObj.isAnyPlayerWithinRangeAt((double)intValue, (double)secondIntValue, (double)thirdIntValue, 7.0D) && this.worldObj.checkNoEntityCollision(entityzombie.getEntityBoundingBox(), entityzombie) && this.worldObj.getCollidingBoundingBoxes(entityzombie, entityzombie.getEntityBoundingBox()).isEmpty() && !this.worldObj.isAnyLiquid(entityzombie.getEntityBoundingBox()))
                            {
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata)
    {
        ...
        double doubleValue = this.rand.nextDouble() * 1.5D * (double)f;

        if (doubleValue > 1.0D)
        {
            this.getEntityAttribute(SharedMonsterAttributes.followRange).applyModifier(new AttributeModifier("Random zombie-spawn bonus", doubleValue, 2));
        }
    ...
    protected int getConversionTimeBoost()
    {
        ...
        if (this.rand.nextFloat() < 0.01F)
        {
            ...
            for (int k = (int)this.posX - 4; k < (int)this.posX + 4 && j < 14; ++k)
            {
                ...
                {
                    for (int index = (int)this.posZ - 4; index < (int)this.posZ + 4 && j < 14; ++index)
                    {
                        Block block = this.worldObj.getBlockState(blockpos$mutableblockpos.set(k, l, index)).getBlock();
