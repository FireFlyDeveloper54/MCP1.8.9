...
public class EntityIronGolem extends EntityGolem
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.motionX * this.motionX + this.motionZ * this.motionZ > 2.500000277905201E-7D && this.rand.nextInt(5) == 0)
        {
            ...
            IBlockState iblockstate = this.worldObj.getBlockState(this.blockSamplePos.set(i, j, k));
    ...
    {
        public AINearestAttackableTargetNonCreeper(final EntityCreature creature, Class<T> classTarget, int chance, boolean checkSight, boolean nearbyOnly, final Predicate <? super T > targetSelector)
        {
            super(creature, classTarget, chance, checkSight, nearbyOnly, targetSelector);
            this.targetEntitySelector = new Predicate<T>()
            {
                public boolean apply(T target)
                {
                    ...
                    else
                    {
                        ...
                        {
                            double targetDistanceLimit = AINearestAttackableTargetNonCreeper.this.getTargetDistance();

                            if (target.isSneaking())
                            {
                                targetDistanceLimit *= 0.800000011920929D;
                            }

                            if (target.isInvisible())
                            {
                                float armorVisibility = ((EntityPlayer)target).getArmorVisibility();
                                ...
                                targetDistanceLimit *= (double)(0.7F * armorVisibility);
                            }

                            if ((double)target.getDistanceToEntity(creature) > targetDistanceLimit)
                            {
                                return false;
                            }
                        }

                        return AINearestAttackableTargetNonCreeper.this.isSuitableTarget(target, false);
                    }
                }
            };
        }
