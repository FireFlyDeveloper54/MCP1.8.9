...
public class EntityWitherSkull extends EntityFireball
{
    ...
    {
        float resistance = super.getExplosionResistance(explosionIn, worldIn, pos, blockStateIn);
        ...
        if (!this.worldObj.isRemote)
        {
            if (movingObject.entityHit != null)
            {
                ...
                if (movingObject.entityHit instanceof EntityLivingBase)
                {
                    ...
                    if (witherDuration > 0)
                    {
                        ((EntityLivingBase)movingObject.entityHit).addPotionEffect(new PotionEffect(Potion.wither.id, 20 * witherDuration, 1));
                    }
                }
            }
