...
public class DataWatcher
{
        ...
        for (int i = buffer.readByte(); i != 127; i = buffer.readByte())
        {
            ...
            switch (j)
            {
                ...
                case 6:
                    ...
                    int secondIntValue = buffer.readInt();
                    int thirdIntValue = buffer.readInt();
                    datawatcher$watchableobject = new DataWatcher.WatchableObject(j, k, new BlockPos(l, secondIntValue, thirdIntValue));
                    break;

                case 7:
                    float f = buffer.readFloat();
                    float secondFloatValue = buffer.readFloat();
                    float thirdFloatValue = buffer.readFloat();
                    datawatcher$watchableobject = new DataWatcher.WatchableObject(j, k, new Rotations(f, secondFloatValue, thirdFloatValue));
            }
    ...
    public void updateWatchedObjectsFromList(List<DataWatcher.WatchableObject> watchedObjectsList)
    {
        this.lock.writeLock().lock();

        for (DataWatcher.WatchableObject datawatcher$watchableobject : watchedObjectsList)
        {
