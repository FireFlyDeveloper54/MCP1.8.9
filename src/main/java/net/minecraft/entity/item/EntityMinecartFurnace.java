...
public class EntityMinecartFurnace extends EntityMinecart
{
    ...
    public void onUpdate()
    {
        ...
        {
            this.worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX, this.posY + 0.8D, this.posZ, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
        }
    }
    ...
    {
        super.moveAlongTrack(railPos, railState);
        double doubleValue = this.pushX * this.pushX + this.pushZ * this.pushZ;

        if (doubleValue > 1.0E-4D && this.motionX * this.motionX + this.motionZ * this.motionZ > 0.001D)
        {
            doubleValue = (double)MathHelper.sqrt_double(doubleValue);
            this.pushX /= doubleValue;
            this.pushZ /= doubleValue;
            ...
            {
                double doubleValue2 = doubleValue / this.getMaximumSpeed();
                this.pushX *= doubleValue2;
                this.pushZ *= doubleValue2;
            }
        }
    }
    ...
    protected void applyDrag()
    {
        ...
        if (doubleValue > 1.0E-4D)
        {
            ...
            this.pushX /= doubleValue;
            this.pushZ /= doubleValue;
            double doubleValue2 = 1.0D;
            this.motionX *= 0.800000011920929D;
            this.motionY *= 0.0D;
            this.motionZ *= 0.800000011920929D;
            this.motionX += this.pushX * doubleValue2;
            this.motionZ += this.pushZ * doubleValue2;
        }
        else
        {
