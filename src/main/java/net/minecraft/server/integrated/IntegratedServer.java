...
public class IntegratedServer extends MinecraftServer
{
    ...
    protected void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, String worldNameIn2)
    {
        ...
        for (int l = 0; l < this.worldServers.length; ++l)
        {
            ...
            if (l == 0)
            {
                if (this.isDemo())
                {
                    this.worldServers[l] = (WorldServer)(new DemoWorldServer(this, isavehandler, worldinfo, dimensionId, this.theProfiler)).init();
                }
                else
                {
                    this.worldServers[l] = (WorldServer)(new WorldServer(this, isavehandler, worldinfo, dimensionId, this.theProfiler)).init();
                }
            ...
            {
                this.worldServers[l] = (WorldServer)(new WorldServerMulti(this, isavehandler, dimensionId, this.worldServers[0], this.theProfiler)).init();
            }
    ...
    public DifficultyInstance getDifficultyAsync(World world, BlockPos pos)
    {
        this.difficultyUpdateWorld = world;
        this.difficultyUpdatePos = pos;
        return this.difficultyLast;
    }

    private void onTick(WorldServer worldServer)
    {
        if (!Config.isTimeDefault())
        {
            this.fixWorldTime(worldServer);
        }

        if (!Config.isWeatherEnabled())
        {
            this.fixWorldWeather(worldServer);
        }

        if (Config.waterOpacityChanged)
        {
            Config.waterOpacityChanged = false;
            ClearWater.updateWaterOpacity(Config.getGameSettings(), worldServer);
        }

        if (this.difficultyUpdateWorld == worldServer && this.difficultyUpdatePos != null)
        {
            this.difficultyLast = worldServer.getDifficultyForLocation(this.difficultyUpdatePos);
            this.difficultyUpdateWorld = null;
            this.difficultyUpdatePos = null;
        }
    }

    private void fixWorldWeather(WorldServer worldServer)
    {
        ...
        if (worldInfo.isRaining() || worldInfo.isThundering())
        {
            ...
            worldServer.setRainStrength(0.0F);
            worldInfo.setThunderTime(0);
            worldInfo.setThundering(false);
            worldServer.setThunderStrength(0.0F);
    ...
    private void fixWorldTime(WorldServer worldServer)
    {
        WorldInfo worldInfo = worldServer.getWorldInfo();

        if (worldInfo.getGameType().getID() == 1)
        {
            long i = worldServer.getWorldTime();
            long j = i % 24000L;
            ...
            if (Config.isTimeDayOnly())
            {
                if (j <= 1000L)
                {
                    worldServer.setWorldTime(i - j + 1001L);
                }

                if (j >= 11000L)
                {
                    worldServer.setWorldTime(i - j + 24001L);
                }
            }
            ...
            if (Config.isTimeNightOnly())
            {
                if (j <= 14000L)
                {
                    worldServer.setWorldTime(i - j + 14001L);
                }

                if (j >= 22000L)
                {
                    worldServer.setWorldTime(i - j + 24000L + 14001L);
                }
            }
        }
    }
