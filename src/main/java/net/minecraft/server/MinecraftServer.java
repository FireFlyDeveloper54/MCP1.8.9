...
public abstract class MinecraftServer implements Runnable, ICommandSender, IThreadListener, IPlayerUsage
{
    ...
    protected void initialWorldChunkLoad()
    {
        ...
        logger.info("Preparing start region for level " + secondIntValue2);
        WorldServer worldServer = this.worldServers[secondIntValue2];
        BlockPos blockPos = worldServer.getSpawnPoint();
        long longValue = getCurrentTimeMillis();

        for (int outerIndex = -192; outerIndex <= 192 && this.isServerRunning(); outerIndex += 16)
        {
            for (int index = -192; index <= 192 && this.isServerRunning(); index += 16)
            {
                long longValue2 = getCurrentTimeMillis();

                if (longValue2 - longValue > 1000L)
                {
                    this.outputPercentRemaining("Preparing spawn area", intValue2 * 100 / 625);
                    longValue = longValue2;
                }

                ++intValue2;
                worldServer.theChunkProviderServer.loadChunk(blockPos.getX() + outerIndex >> 4, blockPos.getZ() + index >> 4);
            }
        }
    ...
    private void addFaviconToStatusResponse(ServerStatusResponse response)
    {
        ...
        if (file1.isFile())
        {
            ...
            try
            {
                ...
                encodedIcon = Base64.encode(byteBuf);
                response.setFavicon("data:image/png;base64," + encodedIcon.toString(Charsets.UTF_8));
            }
    ...
    public void updateTimeLightAndEntities()
    {
        ...
        for (int j = 0; j < this.worldServers.length; ++j)
        {
            ...
            if (j == 0 || this.getAllowNether())
            {
                ...
                this.theProfiler.endStartSection("lighting");
                worldServer.getLightingEngine().processLightUpdates();
                this.theProfiler.endSection();
    ...
    public List<String> getTabCompletions(ICommandSender sender, String input, BlockPos pos)
    {
        ...
        else
        {
            ...
            for (String text3 : this.serverConfigManager.getAllUsernames())
            {
                if (CommandBase.doesStringStartWith(s, text3))
                {
                    list.add(text3);
                }
            }
