...
public abstract class BiomeGenBase
{
    ...
    public static final BiomeGenBase coldTaiga = (new BiomeGenTaiga(30, 0)).setColor(3233098).setBiomeName("Cold Taiga").setFillerBlockMetadata(5159473).setEnableSnow().setTemperatureRainfall(-0.5F, 0.4F).setHeight(height_MidPlains).setDisplayColor(16777215);
    public static final BiomeGenBase coldTaigaHills = (new BiomeGenTaiga(31, 0)).setColor(2375478).setBiomeName("Cold Taiga Hills").setFillerBlockMetadata(5159473).setEnableSnow().setTemperatureRainfall(-0.5F, 0.4F).setHeight(height_LowHills).setDisplayColor(16777215);
    ...
    public static final BiomeGenBase DEFAULT_BIOME = ocean;
    ...
    {
        this.setColor(colorIn, false);
        return this;
    }

    protected BiomeGenBase setDisplayColor(int displayColorIn)
    {
    ...
    protected BiomeGenBase setColor(int colorIn, boolean darkenDisplayColor)
    {
        ...
        {
            this.displayColor = (colorIn & 16711422) >> 1;
        }
        else
        {
            this.displayColor = colorIn;
        }
    ...
    public int getSkyColorByTemp(float temperature)
    {
        ...
        temperature = MathHelper.clamp_float(temperature, -1.0F, 1.0F);
    ...
    public final float getFloatTemperature(BlockPos pos)
    {
        if (pos.getY() > 64)
        {
            float temperatureOffset = (float)(temperatureNoise.getValue((double)pos.getX() * 1.0D / 8.0D, (double)pos.getZ() * 1.0D / 8.0D) * 4.0D);
            return this.temperature - (temperatureOffset + (float)pos.getY() - 64.0F) * 0.05F / 30.0F;
        }
        else
        {
    ...
    public int getGrassColorAtPos(BlockPos pos)
    {
        ...
        double rainfall = (double)MathHelper.clamp_float(this.getFloatRainfall(), 0.0F, 1.0F);
        return ColorizerGrass.getGrassColor(temperature, rainfall);
    }
    ...
    public int getFoliageColorAtPos(BlockPos pos)
    {
        double temperature = (double)MathHelper.clamp_float(this.getFloatTemperature(pos), 0.0F, 1.0F);
        double rainfall = (double)MathHelper.clamp_float(this.getFloatRainfall(), 0.0F, 1.0F);
        return ColorizerFoliage.getFoliageColor(temperature, rainfall);
    }
    ...
    public final void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal)
    {
        ...
        for (int innerIndex = 255; innerIndex >= 0; --innerIndex)
        {
            if (innerIndex <= rand.nextInt(5))
            {
                chunkPrimerIn.setBlockState(localZ, innerIndex, localX, Blocks.bedrock.getDefaultState());
            }
            else
            {
                IBlockState currentState = chunkPrimerIn.getBlockState(localZ, innerIndex, localX);
                ...
                else if (currentState.getBlock() == Blocks.stone)
                {
                    if (remainingFillerDepth == -1)
                    {
                        ...
                        else if (innerIndex >= seaLevel - 4 && innerIndex <= seaLevel + 1)
                        {
                            topState = this.topBlock;
                            fillerState = this.fillerBlock;
                        }

                        if (innerIndex < seaLevel && (topState == null || topState.getBlock().getMaterial() == Material.air))
                        {
                            if (this.getFloatTemperature(mutablePos.set(x, innerIndex, z)) < 0.15F)
                            {
                        ...
                        if (innerIndex >= seaLevel - 1)
                        {
                            chunkPrimerIn.setBlockState(localZ, innerIndex, localX, topState);
                        }
                        else if (innerIndex < seaLevel - 7 - surfaceDepth)
                        {
                            topState = null;
                            fillerState = Blocks.stone.getDefaultState();
                            chunkPrimerIn.setBlockState(localZ, innerIndex, localX, Blocks.gravel.getDefaultState());
                        }
                        else
                        {
                            chunkPrimerIn.setBlockState(localZ, innerIndex, localX, fillerState);
                        }
                    }
                    else if (remainingFillerDepth > 0)
                    {
                        ...
                        if (remainingFillerDepth == 0 && fillerState.getBlock() == Blocks.sand)
                        {
                            remainingFillerDepth = rand.nextInt(4) + Math.max(0, innerIndex - 63);
    ...
    {
        return new BiomeGenMutated(newBiomeId, this);
    }
