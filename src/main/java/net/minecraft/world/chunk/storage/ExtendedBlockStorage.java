...
public class ExtendedBlockStorage
{
    ...
    {
        IBlockState blockState = (IBlockState)Block.BLOCK_STATE_IDS.getByValue(this.data[y << 8 | z << 4 | x]);
    ...
    public void set(int x, int y, int z, IBlockState state)
    {
        ...
        IBlockState oldState = this.get(x, y, z);
    ...
    {
        IBlockState blockState = this.get(x, y, z);
    ...
    public void removeInvalidBlocks()
    {
        ...
        int randomTickBlockCount = 0;
        char[] blockData = this.data;

        for (int i = 0; i < 4096; ++i)
        {
            char blockId = blockData[i];

            if (blockId != 0)
            {
                IBlockState blockState = (IBlockState)Block.BLOCK_STATE_IDS.getByValue(blockId);

                if (blockState != null && blockState.getBlock() != Blocks.air)
                {
                    ++nonAirBlockCount;

                    if (blockState.getBlock().getTickRandomly())
                    {
        ...
        this.tickRefCount = randomTickBlockCount;
    }
