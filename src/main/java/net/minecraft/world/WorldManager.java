...
public class WorldManager implements IWorldAccess
{
    ...
    public void sendBlockBreakProgress(int breakerId, BlockPos pos, int progress)
    {
        for (EntityPlayerMP entityPlayerMP : this.mcServer.getConfigurationManager().getPlayerList())
        {
            ...
            {
                double xCoordinate = (double)pos.getX() - entityPlayerMP.posX;
                double yCoordinate = (double)pos.getY() - entityPlayerMP.posY;
                double zCoordinate = (double)pos.getZ() - entityPlayerMP.posZ;

                if (xCoordinate * xCoordinate + yCoordinate * yCoordinate + zCoordinate * zCoordinate < 1024.0D)
                {
