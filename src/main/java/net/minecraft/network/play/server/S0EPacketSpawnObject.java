...
public class S0EPacketSpawnObject implements Packet<INetHandlerPlayClient>
{
    ...
    public S0EPacketSpawnObject(Entity entityIn, int typeIn, int dataIn)
    {
        ...
        if (dataIn > 0)
        {
            ...
            double motionLimit = 3.9D;

            if (motionX < -motionLimit)
            {
                motionX = -motionLimit;
            }

            if (motionY < -motionLimit)
            {
                motionY = -motionLimit;
            }

            if (motionZ < -motionLimit)
            {
                motionZ = -motionLimit;
            }
            ...
            this.speedX = (int)(motionX * 8000.0D);
            this.speedY = (int)(motionY * 8000.0D);
            this.speedZ = (int)(motionZ * 8000.0D);
        }
    }
