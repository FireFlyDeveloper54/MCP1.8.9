...
public class CommandOp extends CommandBase
{
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos)
    {
        if (args.length == 1)
        {
            String namePrefix = args[args.length - 1];
            List<String> list = Lists.<String>newArrayList();

            for (GameProfile gameProfile : MinecraftServer.getServer().getGameProfiles())
            {
                if (!MinecraftServer.getServer().getConfigurationManager().canSendCommands(gameProfile) && doesStringStartWith(namePrefix, gameProfile.getName()))
                {
