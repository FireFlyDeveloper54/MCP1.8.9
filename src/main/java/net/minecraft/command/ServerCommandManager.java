...
public class ServerCommandManager extends CommandHandler implements IAdminCommand
{
    ...
    {
        boolean shouldBroadcastToOps = true;
        MinecraftServer minecraftServer = MinecraftServer.getServer();

        if (!sender.sendCommandFeedback())
        {
            shouldBroadcastToOps = false;
        }

        IChatComponent adminMessage = new ChatComponentTranslation("chat.type.admin", new Object[] {sender.getName(), new ChatComponentTranslation(msgFormat, msgParams)});
        adminMessage.getChatStyle().setColor(EnumChatFormatting.GRAY);
        adminMessage.getChatStyle().setItalic(Boolean.valueOf(true));

        if (shouldBroadcastToOps)
        {
            for (EntityPlayer entityPlayer : minecraftServer.getConfigurationManager().getPlayerList())
            {
                ...
                {
                    boolean broadcastConsoleToOps = sender instanceof MinecraftServer && MinecraftServer.getServer().shouldBroadcastConsoleToOps();
                    boolean broadcastRconToOps = sender instanceof RConConsoleSource && MinecraftServer.getServer().shouldBroadcastRconToOps();
        ...
        boolean shouldSendFeedback = minecraftServer.worldServers[0].getGameRules().getBoolean("sendCommandFeedback");

        if (sender instanceof CommandBlockLogic)
        {
            shouldSendFeedback = ((CommandBlockLogic)sender).shouldTrackOutput();
        }

        if ((flags & 1) != 1 && shouldSendFeedback || sender instanceof MinecraftServer)
        {
