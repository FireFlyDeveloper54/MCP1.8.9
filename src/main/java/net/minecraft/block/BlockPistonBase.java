...
public class BlockPistonBase extends Block
{
    ...
    public boolean onBlockEventReceived(World worldIn, BlockPos pos, IBlockState state, int eventID, int eventParam)
    {
        ...
        else if (eventID == 1)
        {
            ...
            if (this.isSticky)
            {
                ...
                boolean isExtendingPiston = false;
                ...
                if (block == Blocks.piston_extension)
                {
                    ...
                    if (tileentity instanceof TileEntityPiston)
                    {
                        ...
                        if (tileEntityPiston.getFacing() == enumfacing && tileEntityPiston.isExtending())
                        {
                            ...
                            isExtendingPiston = true;
                        }
                    }
                }

                if (!isExtendingPiston && block.getMaterial() != Material.air && canPush(block, worldIn, blockPos, enumfacing.getOpposite(), false) && (block.getMobilityFlag() == 0 || block == Blocks.piston || block == Blocks.sticky_piston))
                {
    ...
    public static EnumFacing getFacingFromEntity(World worldIn, BlockPos clickedBlock, EntityLivingBase entityIn)
    {
        ...
        {
            double eyeY = entityIn.posY + (double)entityIn.getEyeHeight();

            if (eyeY - (double)clickedBlock.getY() > 2.0D)
            {
                return EnumFacing.UP;
            }

            if ((double)clickedBlock.getY() - eyeY > 0.0D)
            {
    ...
    private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending)
    {
        ...
        else
        {
            ...
            for (int moveIndex = list.size() - 1; moveIndex >= 0; --moveIndex)
            {
                worldIn.notifyNeighborsOfStateChange((BlockPos)list.get(moveIndex), ablock[i++]);
            }
