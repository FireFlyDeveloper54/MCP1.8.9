    ...
    public void checkWitherSpawn(World worldIn, BlockPos pos, TileEntitySkull te)
    {
        if (te.getSkullType() == 1 && pos.getY() >= 2 && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL && !worldIn.isRemote)
        {
            ...
            if (blockpattern$patternhelper != null)
            {
                ...
                for (int l = 0; l < 120; ++l)
                {
                    worldIn.spawnParticle(EnumParticleTypes.SNOWBALL, (double)blockpos.getX() + worldIn.rand.nextDouble(), (double)(blockpos.getY() - 2) + worldIn.rand.nextDouble() * 3.9D, (double)blockpos.getZ() + worldIn.rand.nextDouble(), 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
                }

                for (int palmIndex = 0; palmIndex < blockPattern.getPalmLength(); ++palmIndex)
                {
                    for (int innerIndex2 = 0; innerIndex2 < blockPattern.getThumbLength(); ++innerIndex2)
                    {
                        BlockWorldState blockworldstate2 = blockpattern$patternhelper.translateOffset(palmIndex, innerIndex2, 0);
