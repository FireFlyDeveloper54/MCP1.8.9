...
public class BlockSponge extends Block
{
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (((Boolean)state.getValue(WET)).booleanValue())
        {
            ...
            {
                double particleX = (double)pos.getX();
                double particleY = (double)pos.getY();
                double particleZ = (double)pos.getZ();

                if (enumfacing == EnumFacing.DOWN)
                {
                    particleY = particleY - 0.05D;
                    particleX += rand.nextDouble();
                    particleZ += rand.nextDouble();
                }
                else
                {
                    particleY = particleY + rand.nextDouble() * 0.8D;
                ...
                worldIn.spawnParticle(EnumParticleTypes.DRIP_WATER, particleX, particleY, particleZ, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
            }
        }
    }
}
