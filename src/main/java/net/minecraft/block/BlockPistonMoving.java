...
public class BlockPistonMoving extends BlockContainer
{
    ...
    public AxisAlignedBB getBoundingBox(World worldIn, BlockPos pos, IBlockState extendingBlock, float progress, EnumFacing direction)
    {
        if (extendingBlock.getBlock() != this && extendingBlock.getBlock().getMaterial() != Material.air)
        {
            ...
            {
                double doubleValue = axisAlignedBB.minX;
                double doubleValue2 = axisAlignedBB.minY;
                double doubleValue3 = axisAlignedBB.minZ;
                double doubleValue4 = axisAlignedBB.maxX;
                double doubleValue5 = axisAlignedBB.maxY;
                double doubleValue6 = axisAlignedBB.maxZ;

                if (direction.getFrontOffsetX() < 0)
                {
                    doubleValue -= (double)((float)direction.getFrontOffsetX() * progress);
                }
                else
                {
                    doubleValue4 -= (double)((float)direction.getFrontOffsetX() * progress);
                }

                if (direction.getFrontOffsetY() < 0)
                {
                    doubleValue2 -= (double)((float)direction.getFrontOffsetY() * progress);
                }
                else
                {
                    doubleValue5 -= (double)((float)direction.getFrontOffsetY() * progress);
                }

                if (direction.getFrontOffsetZ() < 0)
                {
                    doubleValue3 -= (double)((float)direction.getFrontOffsetZ() * progress);
                }
                else
                {
                    doubleValue6 -= (double)((float)direction.getFrontOffsetZ() * progress);
                }

                return new AxisAlignedBB(doubleValue, doubleValue2, doubleValue3, doubleValue4, doubleValue5, doubleValue6);
            }
        }
        else
        {
