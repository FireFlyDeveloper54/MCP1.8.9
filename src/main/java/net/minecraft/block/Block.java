...
public class Block
{
    ...
    public static void spawnAsEntity(World worldIn, BlockPos pos, ItemStack stack)
    {
        ...
        {
            float f = 0.5F;
            double doubleValue = (double)(worldIn.rand.nextFloat() * f) + (double)(1.0F - f) * 0.5D;
            double doubleValue2 = (double)(worldIn.rand.nextFloat() * f) + (double)(1.0F - f) * 0.5D;
            double doubleValue3 = (double)(worldIn.rand.nextFloat() * f) + (double)(1.0F - f) * 0.5D;
            EntityItem entityItem = new EntityItem(worldIn, (double)pos.getX() + doubleValue, (double)pos.getY() + doubleValue2, (double)pos.getZ() + doubleValue3, stack);
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end)
    {
        ...
        if (!this.isVecInsideYZBounds(minXIntersection))
        {
            minXIntersection = null;
        }

        if (!this.isVecInsideYZBounds(maxXIntersection))
        {
            maxXIntersection = null;
        }

        if (!this.isVecInsideXZBounds(minYIntersection))
        {
            minYIntersection = null;
        }

        if (!this.isVecInsideXZBounds(maxYIntersection))
        {
            maxYIntersection = null;
        }

        if (!this.isVecInsideXYBounds(minZIntersection))
        {
            minZIntersection = null;
        }

        if (!this.isVecInsideXYBounds(maxZIntersection))
        {
        ...
        if (minXIntersection != null && (closestIntersection == null || start.squareDistanceTo(minXIntersection) < start.squareDistanceTo(closestIntersection)))
        {
        ...
        else
        {
            ...
            if (closestIntersection == maxXIntersection)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (closestIntersection == minYIntersection)
            {
                enumfacing = EnumFacing.DOWN;
            }

            if (closestIntersection == maxYIntersection)
            {
                enumfacing = EnumFacing.UP;
            }

            if (closestIntersection == minZIntersection)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (closestIntersection == maxZIntersection)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            return new MovingObjectPosition(closestIntersection.addVector((double)pos.getX(), (double)pos.getY(), (double)pos.getZ()), enumfacing, pos);
        }
    }
