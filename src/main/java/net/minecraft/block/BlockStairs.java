...
public class BlockStairs extends Block
{
    ...
    private static final int[][] RAYTRACE_PASS_EXCLUSIONS = new int[][] {{4, 5}, {5, 7}, {6, 7}, {4, 6}, {0, 1}, {1, 3}, {2, 3}, {0, 2}};
    ...
    public int getOuterStairShapeCode(IBlockAccess blockAccess, BlockPos pos)
    {
    ...
    public int getInnerStairShapeCode(IBlockAccess blockAccess, BlockPos pos)
    {
        IBlockState iblockstate = blockAccess.getBlockState(pos);
        EnumFacing enumfacing = (EnumFacing)iblockstate.getValue(FACING);
        BlockStairs.EnumHalf blockstairs$enumhalf = (BlockStairs.EnumHalf)iblockstate.getValue(HALF);
    ...
    public boolean setPrimaryStairBounds(IBlockAccess blockAccess, BlockPos pos)
    {
        IBlockState iblockstate = blockAccess.getBlockState(pos);
        EnumFacing enumfacing = (EnumFacing)iblockstate.getValue(FACING);
        BlockStairs.EnumHalf blockstairs$enumhalf = (BlockStairs.EnumHalf)iblockstate.getValue(HALF);
        ...
        float fifthFloatValue = 1.0F;
        float seventhFloatValue = 0.0F;
        float tenthFloatValue = 0.5F;
        boolean flag1 = true;

        if (enumfacing == EnumFacing.EAST)
        {
            fourthFloatValue = 0.5F;
            tenthFloatValue = 1.0F;
            IBlockState iblockstate1 = blockAccess.getBlockState(pos.east());
            Block block = iblockstate1.getBlock();
        ...
        this.setBlockBounds(fourthFloatValue, f, seventhFloatValue, fifthFloatValue, secondFloatValue, tenthFloatValue);
        return flag1;
    }

    public boolean setSecondaryStairBounds(IBlockAccess blockAccess, BlockPos pos)
    {
        IBlockState iblockstate = blockAccess.getBlockState(pos);
        EnumFacing enumfacing = (EnumFacing)iblockstate.getValue(FACING);
        BlockStairs.EnumHalf blockstairs$enumhalf = (BlockStairs.EnumHalf)iblockstate.getValue(HALF);
        ...
        float sixthFloatValue = 0.5F;
        float eighthFloatValue = 0.5F;
        float ninthFloatValue = 1.0F;
        boolean flag1 = false;
        ...
        else if (enumfacing == EnumFacing.WEST)
        {
            ...
            if (isBlockStairs(block1) && blockstairs$enumhalf == iblockstate2.getValue(HALF))
            {
                thirdFloatValue = 0.5F;
                sixthFloatValue = 1.0F;
                EnumFacing enumfacing2 = (EnumFacing)iblockstate2.getValue(FACING);
        ...
        if (flag1)
        {
            this.setBlockBounds(thirdFloatValue, f, eighthFloatValue, sixthFloatValue, floatValue, ninthFloatValue);
        }
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        boolean flag = this.setPrimaryStairBounds(worldIn, pos);
        super.addCollisionBoxesToList(worldIn, pos, state, mask, list, collidingEntity);

        if (flag && this.setSecondaryStairBounds(worldIn, pos))
        {
            super.addCollisionBoxesToList(worldIn, pos, state, mask, list, collidingEntity);
        }
    ...
    public MovingObjectPosition collisionRayTrace(World worldIn, BlockPos pos, Vec3 start, Vec3 end)
    {
        ...
        int[] aint = RAYTRACE_PASS_EXCLUSIONS[i + (flag ? 4 : 0)];
        ...
        for (MovingObjectPosition movingobjectposition : amovingobjectposition)
        {
            ...
            {
                double doubleValue2 = movingobjectposition.hitVec.squareDistanceTo(end);
    ...
    {
        if (this.setPrimaryStairBounds(worldIn, pos))
        {
            switch (this.getInnerStairShapeCode(worldIn, pos))
            {
                case 0:
        ...
        {
            switch (this.getOuterStairShapeCode(worldIn, pos))
            {
                case 0:
                    state = state.withProperty(SHAPE, BlockStairs.EnumShape.STRAIGHT);
                    break;
