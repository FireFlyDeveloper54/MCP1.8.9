...
public class BlockRedstoneWire extends Block
{
    ...
    private IBlockState calculateCurrentChanges(World worldIn, BlockPos secondBlockPos, BlockPos thirdBlockPos, IBlockState state)
    {
        IBlockState iblockstate = state;
        int i = ((Integer)state.getValue(POWER)).intValue();
        int j = 0;
        j = this.getMaxCurrentStrength(worldIn, thirdBlockPos, j);
        this.canProvidePower = false;
        int k = worldIn.isBlockIndirectlyGettingPowered(secondBlockPos);
        ...
        {
            BlockPos blockpos = secondBlockPos.offset(enumfacing);
            boolean flag = blockpos.getX() != thirdBlockPos.getX() || blockpos.getZ() != thirdBlockPos.getZ();
            ...
            if (worldIn.getBlockState(blockpos).getBlock().isNormalCube() && !worldIn.getBlockState(secondBlockPos.up()).getBlock().isNormalCube())
            {
                if (flag && secondBlockPos.getY() >= thirdBlockPos.getY())
                {
                    l = this.getMaxCurrentStrength(worldIn, blockpos.up(), l);
                }
            }
            else if (!worldIn.getBlockState(blockpos).getBlock().isNormalCube() && flag && secondBlockPos.getY() <= thirdBlockPos.getY())
            {
        ...
        if (i != j)
        {
            ...
            if (worldIn.getBlockState(secondBlockPos) == iblockstate)
            {
                worldIn.setBlockState(secondBlockPos, state, 2);
            }

            this.blocksNeedingUpdate.add(secondBlockPos);

            for (EnumFacing enumfacing1 : EnumFacing.VALUES)
            {
                this.blocksNeedingUpdate.add(secondBlockPos.offset(enumfacing1));
            }
        }
    ...
    public int getWeakPower(IBlockAccess worldIn, BlockPos pos, IBlockState state, EnumFacing side)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
                {
                    if (this.isPowerSourceAt(worldIn, pos, enumfacing))
                    {
    ...
    private int colorMultiplier(int powerLevel)
    {
        ...
        float fourthFloatValue = f * f * 0.7F - 0.5F;
        float floatValue4 = f * f * 0.6F - 0.7F;
        ...
        int i = MathHelper.clamp_int((int)(floatValue2 * 255.0F), 0, 255);
        int j = MathHelper.clamp_int((int)(fourthFloatValue * 255.0F), 0, 255);
        int k = MathHelper.clamp_int((int)(floatValue4 * 255.0F), 0, 255);
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        if (i != 0)
        {
            double doubleValue = (double)pos.getX() + 0.5D + ((double)rand.nextFloat() - 0.5D) * 0.2D;
            double secondDoubleValue = (double)((float)pos.getY() + 0.0625F);
            double thirdDoubleValue = (double)pos.getZ() + 0.5D + ((double)rand.nextFloat() - 0.5D) * 0.2D;
            float f = (float)i / 15.0F;
            float floatValue = f * 0.6F + 0.4F;
            float secondFloatValue = Math.max(0.0F, f * f * 0.7F - 0.5F);
            float thirdFloatValue = Math.max(0.0F, f * f * 0.6F - 0.7F);
            worldIn.spawnParticle(EnumParticleTypes.REDSTONE, doubleValue, secondDoubleValue, thirdDoubleValue, (double)floatValue, (double)secondFloatValue, (double)thirdFloatValue, EnumParticleTypes.EMPTY_ARGS);
        }
    }
