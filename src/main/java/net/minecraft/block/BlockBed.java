...
public class BlockBed extends BlockDirectional
{
    ...
    public static BlockPos getSafeExitLocation(World worldIn, BlockPos pos, int tries)
    {
        ...
        for (int l = 0; l <= 1; ++l)
        {
            int minX = i - enumfacing.getFrontOffsetX() * l - 1;
            int minZ = k - enumfacing.getFrontOffsetZ() * l - 1;
            int maxX = minX + 2;
            int maxZ = minZ + 2;

            for (int exitX = minX; exitX <= maxX; ++exitX)
            {
                for (int exitZ = minZ; exitZ <= maxZ; ++exitZ)
                {
                    BlockPos blockPos = new BlockPos(exitX, j, exitZ);
