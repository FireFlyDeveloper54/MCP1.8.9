...
{
    private static final Map<World, List<BlockRedstoneTorch.Toggle>> toggles = new WeakHashMap<World, List<BlockRedstoneTorch.Toggle>>();
        ...
        if (this.isOn)
        {
            if (flag)
            {
                ...
                if (this.isBurnedOut(worldIn, pos, true))
                {
                    ...
                    for (int i = 0; i < 5; ++i)
                    {
                        double secondDoubleValue = (double)pos.getX() + rand.nextDouble() * 0.6D + 0.2D;
                        double fifthDoubleValue = (double)pos.getY() + rand.nextDouble() * 0.6D + 0.2D;
                        double sixthDoubleValue = (double)pos.getZ() + rand.nextDouble() * 0.6D + 0.2D;
                        worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, secondDoubleValue, fifthDoubleValue, sixthDoubleValue, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
                    }
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.isOn)
        {
            double thirdDoubleValue = (double)pos.getX() + 0.5D + (rand.nextDouble() - 0.5D) * 0.2D;
            double fourthDoubleValue = (double)pos.getY() + 0.7D + (rand.nextDouble() - 0.5D) * 0.2D;
            double seventhDoubleValue = (double)pos.getZ() + 0.5D + (rand.nextDouble() - 0.5D) * 0.2D;
            EnumFacing enumfacing = (EnumFacing)state.getValue(FACING);

            if (enumfacing.getAxis().isHorizontal())
            {
                EnumFacing enumfacing1 = enumfacing.getOpposite();
                double doubleValue = 0.27D;
                thirdDoubleValue += 0.27D * (double)enumfacing1.getFrontOffsetX();
                fourthDoubleValue += 0.22D;
                seventhDoubleValue += 0.27D * (double)enumfacing1.getFrontOffsetZ();
            }

            worldIn.spawnParticle(EnumParticleTypes.REDSTONE, thirdDoubleValue, fourthDoubleValue, seventhDoubleValue, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
        }
    }
