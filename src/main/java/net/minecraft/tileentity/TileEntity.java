...
public abstract class TileEntity
{
    ...
    {
        double xCoordinate = (double)this.pos.getX() + 0.5D - x;
        double yCoordinate = (double)this.pos.getY() + 0.5D - y;
        double zCoordinate = (double)this.pos.getZ() + 0.5D - z;
        ...
        if (this.worldObj != null)
        {
            ...
            reportCategory.addCrashSectionCallable("Actual block type", new Callable<String>()
            {
                public String call() throws Exception
                {
                    ...
                    try
                    {
                        return String.format(Locale.ROOT, "ID #%d (%s // %s)", new Object[] {Integer.valueOf(i), Block.getBlockById(i).getUnlocalizedName(), Block.getBlockById(i).getClass().getCanonicalName()});
                    }
            ...
            reportCategory.addCrashSectionCallable("Actual block data value", new Callable<String>()
            {
                public String call() throws Exception
                {
                    ...
                    {
                        String s = String.format(Locale.ROOT, "%4s", new Object[] {Integer.toBinaryString(i)}).replace(" ", "0");
                        return String.format(Locale.ROOT, "%1$d / 0x%1$X / 0b%2$s", new Object[] {Integer.valueOf(i), s});
                    }
                }
            });
        }
