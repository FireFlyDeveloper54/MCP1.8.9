...
public class ItemBlock extends Item
{
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else if (worldIn.canBlockBePlaced(this.block, pos, false, side, (Entity)null, stack))
        {
            ...
            IBlockState iblockstate1 = this.block.onBlockPlaced(worldIn, pos, side, hitX, hitY, hitZ, metadata, playerIn);
    ...
    public static boolean setTileEntityNBT(World worldIn, EntityPlayer player, BlockPos pos, ItemStack stack)
    {
        ...
        else
        {
            if (stack.hasTagCompound() && stack.getTagCompound().hasKey("BlockEntityTag", 10))
            {
                ...
                if (tileEntity != null)
                {
                    if (!worldIn.isRemote && tileEntity.onlyOpsCanSetNbt() && !minecraftServer.getConfigurationManager().canSendCommands(player.getGameProfile()))
                    {
                        return false;
                    }
