...
public class ItemSlab extends ItemBlock
{
    ...
    public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack)
    {
        ...
        if (iblockstate.getBlock() == this.singleSlab)
        {
            boolean isTopHalf = iblockstate.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP;

            if ((side == EnumFacing.UP && !isTopHalf || side == EnumFacing.DOWN && isTopHalf) && object == iblockstate.getValue(iproperty))
            {
                return true;
            }
        }
