...
public class RecipesArmor
{
    ...
    public void addRecipes(CraftingManager craftManager)
    {
        ...
        {
            Item materialItem = this.recipeItems[0][materialIndex];

            for (int armorTypeIndex = 0; armorTypeIndex < this.recipeItems.length - 1; ++armorTypeIndex)
            {
                Item armorItem = this.recipeItems[armorTypeIndex + 1][materialIndex];
                craftManager.addRecipe(new ItemStack(armorItem), new Object[] {this.recipePatterns[armorTypeIndex], 'X', materialItem});
            }
        }
    }
}
