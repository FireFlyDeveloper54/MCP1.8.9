...
public class MovingSoundMinecartRiding extends MovingSound
{
    ...
    public void update()
    {
        ...
        {
            float minecartSpeed = MathHelper.sqrt_double(this.minecart.motionX * this.minecart.motionX + this.minecart.motionZ * this.minecart.motionZ);

            if ((double)minecartSpeed >= 0.01D)
            {
                this.volume = 0.0F + MathHelper.clamp_float(minecartSpeed, 0.0F, 1.0F) * 0.75F;
            }
            else
            {
                this.volume = 0.0F;
