...
public class ResourcePackRepository
{
    ...
    private static final FileFilter resourcePackFilter = new FileFilter()
    {
        public boolean accept(File file)
        {
            ...
            boolean isFolderPack = file.isDirectory() && (new File(file, "pack.mcmeta")).isFile();
            return flag || isFolderPack;
        }
    };
    ...
    public ListenableFuture<Object> downloadResourcePack(String url, String hash)
    {
        ...
        try
        {
            ...
            if (file1.exists() && hash.length() == 40)
            {
                try
                {
                    String actualHash = Hashing.sha1().hashBytes(Files.toByteArray(file1)).toString();
                    ...
                    logger.warn("File " + file1 + " had wrong hash (expected " + hash + ", found " + actualHash + "). Deleting it.");
    ...
    public class Entry
    {
        ...
        {
            return this == other ? true : (other instanceof ResourcePackRepository.Entry ? this.toString().equals(other.toString()) : false);
        }
        ...
        {
            return this.resourcePackFile.getName() + ":" + (this.resourcePackFile.isDirectory() ? "folder" : "zip") + ":" + this.resourcePackFile.lastModified();
        }
    }
}
