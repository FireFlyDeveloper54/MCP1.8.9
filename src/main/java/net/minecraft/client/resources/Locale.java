...
public class Locale
{
    ...
    public synchronized void loadLocaleDataFiles(IResourceManager resourceManager, List<String> languageList)
    {
        ...
        {
            String text2 = "lang/" + s + ".lang";

            for (String text3 : resourceManager.getResourceDomains())
            {
                try
                {
                    this.loadLocaleData(resourceManager.getAllResources(new ResourceLocation(text3, text2)));
                }
    ...
    private void loadLocaleData(InputStream inputStreamIn) throws IOException
    {
        for (String s : IOUtils.readLines(inputStreamIn, Charsets.UTF_8))
        {
            if (!s.isEmpty() && s.charAt(0) != 35)
            {
                ...
                {
                    String stringValue = astring[0];
                    String secondStringValue = pattern.matcher(astring[1]).replaceAll("%$1s");
                    this.properties.put(stringValue, secondStringValue);
                }
            }
        }
    }
