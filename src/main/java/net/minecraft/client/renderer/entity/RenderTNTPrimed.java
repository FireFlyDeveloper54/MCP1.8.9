...
public class RenderTNTPrimed extends Render<EntityTNTPrimed>
{
    ...
    public void doRender(EntityTNTPrimed entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        {
            float fuseProgress = 1.0F - ((float)entity.fuse - partialTicks + 1.0F) / 10.0F;
            fuseProgress = MathHelper.clamp_float(fuseProgress, 0.0F, 1.0F);
            fuseProgress = fuseProgress * fuseProgress;
            fuseProgress = fuseProgress * fuseProgress;
            float flashScale = 1.0F + fuseProgress * 0.3F;
            GlStateManager.scale(flashScale, flashScale, flashScale);
        }

        float flashAlpha = (1.0F - ((float)entity.fuse - partialTicks + 1.0F) / 100.0F) * 0.8F;
