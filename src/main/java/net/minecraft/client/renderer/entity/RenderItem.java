...
{
    private static final String[] STACK_SIZE_TEXT = new String[65];
    ...
    private void renderModel(IBakedModel model, int color, ItemStack stack)
    {
        ...
        boolean textureBound = Minecraft.getMinecraft().getTextureMapBlocks().isTextureBound();
        boolean useMultiTexture = Config.isMultiTexture() && textureBound;
        ...
        {
            this.renderQuads(worldRenderer, model.getFaceQuads(facing), color, stack);
        }
        ...
        if (stack != null)
        {
            ...
            else
            {
                ...
                if (this.renderModelHasEmissive)
                {
                    ...
                    OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0F, previousBrightnessY);
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                float firstGlintOffset = (float)(Minecraft.getSystemTime() % 3000L) / 3000.0F / 8.0F;
                GlStateManager.translate(firstGlintOffset, 0.0F, 0.0F);
                GlStateManager.rotate(-50.0F, 0.0F, 0.0F, 1.0F);
                this.renderModel(model, -8372020);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scale(8.0F, 8.0F, 8.0F);
                float secondGlintOffset = (float)(Minecraft.getSystemTime() % 4873L) / 4873.0F / 8.0F;
                GlStateManager.translate(-secondGlintOffset, 0.0F, 0.0F);
    ...
    {
        boolean tintFromStack = color == -1 && stack != null;
        int quadIndex = 0;

        for (int quadCount = quads.size(); quadIndex < quadCount; ++quadIndex)
        {
            BakedQuad bakedQuad = quads.get(quadIndex);
            int quadColor = color;

            if (tintFromStack && bakedQuad.hasTintIndex())
            {
                quadColor = stack.getItem().getColorFromItemStack(stack, bakedQuad.getTintIndex());

                if (Config.isCustomColors())
                {
                    quadColor = CustomColors.getColorFromItemStack(stack, bakedQuad.getTintIndex(), quadColor);
                }
                ...
                quadColor = quadColor | -16777216;
            }

            this.renderQuad(renderer, bakedQuad, quadColor);
        }
    }

    public boolean shouldRenderItemIn3D(ItemStack stack)
    {
        IBakedModel bakedModel = this.itemModelMesher.getItemModel(stack);
    ...
    public void renderItem(ItemStack stack, ItemCameraTransforms.TransformType cameraTransformType)
    {
        ...
        {
            IBakedModel bakedModel = this.itemModelMesher.getItemModel(stack);
            this.renderItemModelTransform(stack, bakedModel, cameraTransformType);
        }
    }
    ...
    public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, String text)
    {
        if (stack != null)
        {
            ...
            {
                String overlayText = text == null ? getStackSizeText(stack.stackSize) : text;

                if (text == null && stack.stackSize < 1)
                {
                    overlayText = EnumChatFormatting.RED + getStackSizeText(stack.stackSize);
                }

                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                GlStateManager.disableBlend();
                fr.drawStringWithShadow(overlayText, (float)(xPosition + 19 - 2 - fr.getStringWidth(overlayText)), (float)(yPosition + 6 + 3), 16777215);
                GlStateManager.enableLighting();
    ...
    {
        return stackSize >= 0 && stackSize < STACK_SIZE_TEXT.length ? STACK_SIZE_TEXT[stackSize] : String.valueOf(stackSize);
    }

    static
    {
        for (int stackSize = 0; stackSize < STACK_SIZE_TEXT.length; ++stackSize)
        {
            STACK_SIZE_TEXT[stackSize] = String.valueOf(stackSize);
        }
    }
    ...
    {
        float baseBlue = (float)(color & 255);
        float baseGreen = (float)(color >>> 8 & 255);
        float baseRed = (float)(color >>> 16 & 255);
        float baseAlpha = (float)(color >>> 24 & 255);
        int[] vertexData = quad.getVertexData();
        int vertexStride = vertexData.length / 4;

        for (int vertexIndex = 0; vertexIndex < 4; ++vertexIndex)
        {
            int vertexColor = vertexData[3 + vertexStride * vertexIndex];
            float vertexBlue = (float)(vertexColor & 255);
            float vertexGreen = (float)(vertexColor >>> 8 & 255);
            float vertexRed = (float)(vertexColor >>> 16 & 255);
            float vertexAlpha = (float)(vertexColor >>> 24 & 255);
            int blendedBlue = Math.min(255, (int)(baseBlue * vertexBlue / 255.0F));
            int blendedGreen = Math.min(255, (int)(baseGreen * vertexGreen / 255.0F));
            int blendedRed = Math.min(255, (int)(baseRed * vertexRed / 255.0F));
            int blendedAlpha = Math.min(255, (int)(baseAlpha * vertexAlpha / 255.0F));
            renderer.putColorRGBA(renderer.getColorIndex(4 - vertexIndex), blendedBlue, blendedGreen, blendedRed, blendedAlpha);
        }
    }
}
