...
public class RenderManager
{
    ...
    {
        return instance != null ? instance.renderPosX : 0.0D;
    }

    public static double getRenderPosY()
    {
        return instance != null ? instance.renderPosY : 0.0D;
    }

    public static double getRenderPosZ()
    {
        return instance != null ? instance.renderPosZ : 0.0D;
    }
    ...
    public <T extends Entity> Render<T> getEntityRenderObject(Entity entityIn)
    {
        if (entityIn instanceof AbstractClientPlayer)
        {
            ...
            RenderPlayer renderPlayer = this.skinMap.get(skinType);
    ...
    public void cacheActiveRenderInfo(World worldIn, FontRenderer textRendererIn, Entity livingPlayerIn, Entity pointedEntityIn, GameSettings optionsIn, float partialTicks)
    {
        ...
        {
            BlockPos bedPos = new BlockPos(livingPlayerIn);
            IBlockState blockState = worldIn.getBlockState(bedPos);
            Block block = blockState.getBlock();

            if (block == Blocks.bed)
            {
                int horizontalIndex = ((EnumFacing)blockState.getValue(BlockBed.FACING)).getHorizontalIndex();
                this.playerViewY = (float)(horizontalIndex * 90 + 180);
    ...
    public boolean renderEntityStatic(Entity entity, float partialTicks, boolean hideDebugBox)
    {
        ...
        double interpolatedX = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)partialTicks;
        double interpolatedY = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)partialTicks;
        double interpolatedZ = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)partialTicks;
        float interpolatedYaw = entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks;
        int packedBrightness = entity.getBrightnessForRender(partialTicks);

        if (entity.isBurning())
        {
            packedBrightness = 15728880;
        }
        ...
        return this.doRenderEntity(entity, interpolatedX - this.renderPosX, interpolatedY - this.renderPosY, interpolatedZ - this.renderPosZ, interpolatedYaw, partialTicks, hideDebugBox);
    }

    public void renderWitherSkull(Entity entityIn, float partialTicks)
    {
        double interpolatedX = entityIn.lastTickPosX + (entityIn.posX - entityIn.lastTickPosX) * (double)partialTicks;
        double interpolatedY = entityIn.lastTickPosY + (entityIn.posY - entityIn.lastTickPosY) * (double)partialTicks;
        double interpolatedZ = entityIn.lastTickPosZ + (entityIn.posZ - entityIn.lastTickPosZ) * (double)partialTicks;
        Render<Entity> render = this.<Entity>getEntityRenderObject(entityIn);

        if (render != null && this.renderEngine != null)
        {
            int packedBrightness = entityIn.getBrightnessForRender(partialTicks);
            int lightmapU = packedBrightness % 65536;
            int lightmapV = packedBrightness / 65536;
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float)lightmapU / 1.0F, (float)lightmapV / 1.0F);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            render.renderName(entityIn, interpolatedX - this.renderPosX, interpolatedY - this.renderPosY, interpolatedZ - this.renderPosZ);
        }
    }
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x, double y, double z, float entityYaw, float partialTicks)
    {
        if (!Shaders.isShadowPass)
        {
            ...
            float halfWidth = entityIn.width / 2.0F;
            AxisAlignedBB entityBounds = entityIn.getEntityBoundingBox();
            AxisAlignedBB renderBounds = new AxisAlignedBB(entityBounds.minX - entityIn.posX + x, entityBounds.minY - entityIn.posY + y, entityBounds.minZ - entityIn.posZ + z, entityBounds.maxX - entityIn.posX + x, entityBounds.maxY - entityIn.posY + y, entityBounds.maxZ - entityIn.posZ + z);
            RenderGlobal.drawOutlinedBoundingBox(renderBounds, 255, 255, 255, 255);

            if (entityIn instanceof EntityLivingBase)
            {
                float eyeLineHalfHeight = 0.01F;
                RenderGlobal.drawOutlinedBoundingBox(new AxisAlignedBB(x - (double)halfWidth, y + (double)entityIn.getEyeHeight() - (double)eyeLineHalfHeight, z - (double)halfWidth, x + (double)halfWidth, y + (double)entityIn.getEyeHeight() + (double)eyeLineHalfHeight, z + (double)halfWidth), 255, 0, 0, 255);
            }

            Tessellator tessellator = Tessellator.getInstance();
            WorldRenderer worldRenderer = tessellator.getWorldRenderer();
            Vec3 lookVector = entityIn.getLook(partialTicks);
            worldRenderer.begin(3, DefaultVertexFormats.POSITION_COLOR);
            worldRenderer.pos(x, y + (double)entityIn.getEyeHeight(), z).color(0, 0, 255, 255).endVertex();
            worldRenderer.pos(x + lookVector.xCoord * 2.0D, y + (double)entityIn.getEyeHeight() + lookVector.yCoord * 2.0D, z + lookVector.zCoord * 2.0D).color(0, 0, 255, 255).endVertex();
            tessellator.draw();
    ...
    {
        double deltaX = x - this.viewerPosX;
        double deltaY = y - this.viewerPosY;
        double deltaZ = z - this.viewerPosZ;
    ...
    public void setEntityRenderMap(Map entityRenderMapIn)
    {
        this.entityRenderMap = entityRenderMapIn;
    }
