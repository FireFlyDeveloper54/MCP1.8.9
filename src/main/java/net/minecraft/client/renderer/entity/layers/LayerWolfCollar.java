...
public class LayerWolfCollar implements LayerRenderer<EntityWolf>
{
    ...
    public void doRenderLayer(EntityWolf entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (entitylivingbaseIn.isTamed() && !entitylivingbaseIn.isInvisible())
        {
            ...
            EnumDyeColor collarColor = EnumDyeColor.byMetadata(entitylivingbaseIn.getCollarColor().getMetadata());
            float[] collarRgb = EntitySheep.getDyeRgb(collarColor);

            if (Config.isCustomColors())
            {
                collarRgb = CustomColors.getWolfCollarColors(collarColor, collarRgb);
            }

            GlStateManager.color(collarRgb[0], collarRgb[1], collarRgb[2]);
