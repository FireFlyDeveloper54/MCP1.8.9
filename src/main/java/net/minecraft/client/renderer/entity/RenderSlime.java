...
public class RenderSlime extends RenderLiving<EntitySlime>
{
    ...
    {
        float slimeSize = (float)entitylivingbaseIn.getSlimeSize();
        float squishFactor = (entitylivingbaseIn.prevSquishFactor + (entitylivingbaseIn.squishFactor - entitylivingbaseIn.prevSquishFactor) * partialTickTime) / (slimeSize * 0.5F + 1.0F);
        float inverseSquishFactor = 1.0F / (squishFactor + 1.0F);
        GlStateManager.scale(inverseSquishFactor * slimeSize, 1.0F / inverseSquishFactor * slimeSize, inverseSquishFactor * slimeSize);
    }
