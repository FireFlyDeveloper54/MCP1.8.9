...
public class LayerWitherAura implements LayerRenderer<EntityWither>
{
    ...
    public void doRenderLayer(EntityWither entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (entitylivingbaseIn.isArmored())
        {
            ...
            float auraTicks = (float)entitylivingbaseIn.ticksExisted + partialTicks;
            float textureOffsetX = MathHelper.cos(auraTicks * 0.02F) * 3.0F;
            float textureOffsetY = auraTicks * 0.01F;
            GlStateManager.translate(textureOffsetX, textureOffsetY, 0.0F);
            GlStateManager.matrixMode(5888);
            GlStateManager.enableBlend();
            float auraBrightness = 0.5F;
            GlStateManager.color(auraBrightness, auraBrightness, auraBrightness, 1.0F);
