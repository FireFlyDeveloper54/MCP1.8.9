...
public class ItemModelMesher
{
    ...
    private final IntHashMap<IBakedModel> simpleShapesCache = new IntHashMap<IBakedModel>();
    ...
    {
        return this.simpleShapesCache.lookup(this.getIndex(item, meta));
    }
    ...
    {
        int index = this.getIndex(item, meta);
        this.simpleShapes.put(Integer.valueOf(index), location);
        this.simpleShapesCache.addKey(index, this.modelManager.getModel(location));
    }
    ...
    {
        this.simpleShapesCache.clearMap();

        for (Entry<Integer, ModelResourceLocation> entry : this.simpleShapes.entrySet())
        {
            this.simpleShapesCache.addKey(entry.getKey().intValue(), this.modelManager.getModel(entry.getValue()));
        }
    }
}
