...
public class ModelBiped extends ModelBase
{
    ...
    public ModelBiped(float modelSize, float yOffset, int textureWidthIn, int textureHeightIn)
    {
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        ...
        if (this.isChild)
        {
            ...
            GlStateManager.scale(1.5F / childScale, 1.5F / childScale, 1.5F / childScale);
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        {
            float swingProgressValue = this.swingProgress;
            this.bipedBody.rotateAngleY = MathHelper.sin(MathHelper.sqrt_float(swingProgressValue) * (float)Math.PI * 2.0F) * 0.2F;
            ...
            float swingEase = 1.0F - this.swingProgress;
            swingEase = swingEase * swingEase;
            swingEase = swingEase * swingEase;
            swingEase = 1.0F - swingEase;
            float swingSin = MathHelper.sin(swingEase * (float)Math.PI);
            float headPitchSwingOffset = MathHelper.sin(this.swingProgress * (float)Math.PI) * -(this.bipedHead.rotateAngleX - 0.7F) * 0.75F;
            this.bipedRightArm.rotateAngleX = (float)((double)this.bipedRightArm.rotateAngleX - ((double)swingSin * 1.2D + (double)headPitchSwingOffset));
        ...
        if (this.aimedBow)
        {
            ...
            this.bipedRightArm.rotateAngleZ = 0.0F;
            this.bipedLeftArm.rotateAngleZ = 0.0F;
            this.bipedRightArm.rotateAngleY = -(0.1F - bowCharge * 0.6F) + this.bipedHead.rotateAngleY;
            this.bipedLeftArm.rotateAngleY = 0.1F - bowCharge * 0.6F + this.bipedHead.rotateAngleY + 0.4F;
            this.bipedRightArm.rotateAngleX = -((float)Math.PI / 2F) + this.bipedHead.rotateAngleX;
            this.bipedLeftArm.rotateAngleX = -((float)Math.PI / 2F) + this.bipedHead.rotateAngleX;
            this.bipedRightArm.rotateAngleX -= bowCharge * 1.2F - bowSwing * 0.4F;
            this.bipedLeftArm.rotateAngleX -= bowCharge * 1.2F - bowSwing * 0.4F;
            this.bipedRightArm.rotateAngleZ += MathHelper.cos(ageInTicks * 0.09F) * 0.05F + 0.05F;
            this.bipedLeftArm.rotateAngleZ -= MathHelper.cos(ageInTicks * 0.09F) * 0.05F + 0.05F;
            this.bipedRightArm.rotateAngleX += MathHelper.sin(ageInTicks * 0.067F) * 0.05F;
            this.bipedLeftArm.rotateAngleX -= MathHelper.sin(ageInTicks * 0.067F) * 0.05F;
