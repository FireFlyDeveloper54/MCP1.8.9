...
public class EntityDropParticleFX extends EntityFX
{
    ...
    protected EntityDropParticleFX(World worldIn, double xCoordIn, double yCoordIn, double zCoordIn, Material materialIn)
    {
        ...
        this.materialType = materialIn;
        this.bobTimer = 40;
        this.particleMaxAge = (int)(64.0D / (this.rand.nextDouble() * 0.8D + 0.2D));
        this.motionX = this.motionY = this.motionZ = 0.0D;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.onGround)
        {
            ...
            {
                this.setDead();
                this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
            }
            else
            {
        ...
        {
            double liquidHeight = 0.0D;

            if (blockState.getBlock() instanceof BlockLiquid)
            {
                liquidHeight = (double)BlockLiquid.getLiquidHeightPercent(((Integer)blockState.getValue(BlockLiquid.LEVEL)).intValue());
            }

            double liquidSurfaceY = (double)(MathHelper.floor_double(this.posY) + 1) - liquidHeight;

            if (this.posY < liquidSurfaceY)
            {
                this.setDead();
            }
        }
