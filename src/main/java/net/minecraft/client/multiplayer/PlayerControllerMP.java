...
public class PlayerControllerMP
{
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec)
    {
        ...
        else
        {
            if (this.currentGameType != WorldSettings.GameType.SPECTATOR)
            {
                ...
                if (!blockActivated && heldStack != null && heldStack.getItem() instanceof ItemBlock)
                {
            ...
            if (!blockActivated && this.currentGameType != WorldSettings.GameType.SPECTATOR)
            {
                if (heldStack == null)
                {
                    return false;
                ...
                {
                    int heldMetadata = heldStack.getMetadata();
                    int heldStackSize = heldStack.stackSize;
                    boolean itemUsed = heldStack.onItemUse(player, worldIn, hitPos, side, hitX, hitY, hitZ);
                    heldStack.setItemDamage(heldMetadata);
                    heldStack.stackSize = heldStackSize;
                    return itemUsed;
                }
                else
                {
    ...
    {
        this.syncCurrentPlayItem();
        Vec3 localValue = new Vec3(movingObject.hitVec.xCoord - entityIn.posX, movingObject.hitVec.yCoord - entityIn.posY, movingObject.hitVec.zCoord - entityIn.posZ);
        this.netClientHandler.addToSendQueue(new C02PacketUseEntity(entityIn, localValue));
        return this.currentGameType != WorldSettings.GameType.SPECTATOR && entityIn.interactAt(player, localValue);
    }
