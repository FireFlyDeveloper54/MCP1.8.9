...
public class WorldClient extends World
{
    ...
    public void invalidateBlockReceiveRegion(int intValue, int thirdIntValue, int fifthIntValue, int secondIntValue, int fourthIntValue, int sixthIntValue)
    {
    }
    ...
    public void doVoidFogParticles(int posX, int posY, int posZ)
    {
        ...
        for (int j = 0; j < 1000; ++j)
        {
            ...
            int intValue2 = posZ + this.rand.nextInt(i) - this.rand.nextInt(i);
            blockpos$mutableblockpos.set(k, l, intValue2);
            ...
            {
                this.spawnParticle(EnumParticleTypes.BARRIER, (double)((float)k + 0.5F), (double)((float)l + 0.5F), (double)((float)intValue2 + 0.5F), 0.0D, 0.0D, 0.0D, EnumParticleTypes.EMPTY_ARGS);
            }
        }
    }
    ...
    public void removeAllEntities()
    {
        ...
        for (int index2 = 0; index2 < this.loadedEntityList.size(); ++index2)
        {
            Entity entity1 = this.loadedEntityList.get(index2);
            ...
            if (entity1.isDead)
            {
                int secondIntValue2 = entity1.chunkCoordX;
                int thirdIntValue2 = entity1.chunkCoordZ;

                if (entity1.addedToChunk && this.isChunkLoaded(secondIntValue2, thirdIntValue2, true))
                {
                    this.getChunkFromChunkCoords(secondIntValue2, thirdIntValue2).removeEntity(entity1);
                }

                this.loadedEntityList.remove(index2--);
    ...
    {
        double doubleValue = this.mc.getRenderViewEntity().getDistanceSq(x, y, z);
        ...
        if (distanceDelay && doubleValue > 100.0D)
        {
            double doubleValue2 = MathHelper.fastSqrt_double(doubleValue) / 40.0D;
            this.mc.getSoundHandler().playDelayedSound(positionedSoundRecord, (int)(doubleValue2 * 20.0D));
        }
        else
        {
