...
public class GuiControls extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        for (GameSettings.Options option : optionsArr)
        {
            if (option.getEnumFloat())
            {
                this.buttonList.add(new GuiOptionSlider(option.returnEnumOrdinal(), this.width / 2 - 155 + optionIndex % 2 * 160, 18 + 24 * (optionIndex >> 1), option));
            }
            else
            {
                this.buttonList.add(new GuiOptionButton(option.returnEnumOrdinal(), this.width / 2 - 155 + optionIndex % 2 * 160, 18 + 24 * (optionIndex >> 1), option, this.options.getKeyBinding(option)));
            }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        boolean allKeysDefault = true;

        for (KeyBinding keyBinding : this.options.keyBindings)
        {
            if (keyBinding.getKeyCode() != keyBinding.getKeyCodeDefault())
            {
                allKeysDefault = false;
                break;
            }
        }

        this.buttonReset.enabled = !allKeysDefault;
