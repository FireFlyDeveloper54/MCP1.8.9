        ...
        private void render(boolean noOverlayRendering)
        {
            ...
            for (Vec4b vec4b : this.mapData.mapDecorations.values())
            {
                if (!noOverlayRendering || vec4b.getX() == 1)
                {
                    ...
                    byte byteValue = vec4b.getX();
                    float floatValue2 = (float)(byteValue % 4 + 0) / 4.0F;
                    float floatValue3 = (float)(byteValue / 4 + 0) / 4.0F;
                    float floatValue4 = (float)(byteValue % 4 + 1) / 4.0F;
                    float floatValue5 = (float)(byteValue / 4 + 1) / 4.0F;
                    worldRenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
                    float floatValue6 = -0.001F;
                    worldRenderer.pos(-1.0D, 1.0D, (double)((float)k * -0.001F)).tex((double)floatValue2, (double)floatValue3).endVertex();
                    worldRenderer.pos(1.0D, 1.0D, (double)((float)k * -0.001F)).tex((double)floatValue4, (double)floatValue3).endVertex();
                    worldRenderer.pos(1.0D, -1.0D, (double)((float)k * -0.001F)).tex((double)floatValue4, (double)floatValue5).endVertex();
                    worldRenderer.pos(-1.0D, -1.0D, (double)((float)k * -0.001F)).tex((double)floatValue2, (double)floatValue5).endVertex();
                    tessellator.draw();
