...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    protected void drawHoveringText(List<String> textLines, int x, int y)
    {
        if (!textLines.isEmpty())
        {
            ...
            if (intValue + i > this.width)
            {
                intValue -= 28 + i;
            }

            if (intValue2 + k + 6 > this.height)
            {
                intValue2 = this.height - k - 6;
            }

            this.zLevel = 300.0F;
            this.itemRender.zLevel = 300.0F;
            int l = -267386864;
            this.drawGradientRect(intValue - 3, intValue2 - 4, intValue + i + 3, intValue2 - 3, l, l);
            this.drawGradientRect(intValue - 3, intValue2 + k + 3, intValue + i + 3, intValue2 + k + 4, l, l);
            this.drawGradientRect(intValue - 3, intValue2 - 3, intValue + i + 3, intValue2 + k + 3, l, l);
            this.drawGradientRect(intValue - 4, intValue2 - 3, intValue - 3, intValue2 + k + 3, l, l);
            this.drawGradientRect(intValue + i + 3, intValue2 - 3, intValue + i + 4, intValue2 + k + 3, l, l);
            int intValue3 = 1347420415;
            int secondIntValue2 = (intValue3 & 16711422) >> 1 | intValue3 & -16777216;
            this.drawGradientRect(intValue - 3, intValue2 - 3 + 1, intValue - 3 + 1, intValue2 + k + 3 - 1, intValue3, secondIntValue2);
            this.drawGradientRect(intValue + i + 2, intValue2 - 3 + 1, intValue + i + 3, intValue2 + k + 3 - 1, intValue3, secondIntValue2);
            this.drawGradientRect(intValue - 3, intValue2 - 3, intValue + i + 3, intValue2 - 3 + 1, intValue3, intValue3);
            this.drawGradientRect(intValue - 3, intValue2 + k + 2, intValue + i + 3, intValue2 + k + 3, secondIntValue2, secondIntValue2);

            for (int nestedIndex = 0; nestedIndex < textLines.size(); ++nestedIndex)
            {
                String text2 = textLines.get(nestedIndex);
                this.fontRendererObj.drawStringWithShadow(text2, (float)intValue, (float)intValue2, -1);
    ...
    protected void handleComponentHover(IChatComponent component, int x, int y)
    {
        if (component != null && component.getChatStyle().getChatHoverEvent() != null)
        {
            ...
            if (hoverevent.getAction() == HoverEvent.Action.SHOW_ITEM)
            {
                ...
                catch (NBTException caughtNBTException)
                {
                    ;
                }
            ...
            else if (hoverevent.getAction() == HoverEvent.Action.SHOW_ACHIEVEMENT)
            {
                ...
                if (statbase != null)
                {
                    ...
                    String stringValue = statbase instanceof Achievement ? ((Achievement)statbase).getDescription() : null;
                    List<String> list = Lists.newArrayList(new String[] {ichatcomponent.getFormattedText(), ichatcomponent1.getFormattedText()});

                    if (stringValue != null)
                    {
                        list.addAll(this.fontRendererObj.listFormattedStringToWidth(stringValue, 150));
                    }
            ...
            else if (clickevent != null)
            {
                if (clickevent.getAction() == ClickEvent.Action.OPEN_URL)
                {
                    ...
                    try
                    {
                        ...
                        String lowerProtocol = s.toLowerCase(Locale.ROOT);

                        if (!PROTOCOLS.contains(lowerProtocol))
                        {
                            throw new URISyntaxException(clickevent.getValue(), "Unsupported protocol: " + lowerProtocol);
                        }
                ...
                {
                    URI localValue = (new File(clickevent.getValue())).toURI();
                    this.openWebLink(localValue);
                }
                ...
                {
                    ChatUserInfo chatuserinfo = this.mc.getTwitchStream().getChatUserInfo(clickevent.getValue());
