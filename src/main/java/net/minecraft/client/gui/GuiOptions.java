...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback
{
    ...
    public void initGui()
    {
        ...
        {
            GuiOptionButton realmsNotificationsButton = new GuiOptionButton(GameSettings.Options.REALMS_NOTIFICATIONS.returnEnumOrdinal(), this.width / 2 - 155 + optionIndex % 2 * 160, this.height / 6 - 12 + 24 * (optionIndex >> 1), GameSettings.Options.REALMS_NOTIFICATIONS, this.game_settings_1.getKeyBinding(GameSettings.Options.REALMS_NOTIFICATIONS));
            this.buttonList.add(realmsNotificationsButton);
        }
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        if (button.enabled)
        {
            ...
            {
                this.mc.gameSettings.saveOptions();
                IStream stream = this.mc.getTwitchStream();

                if (stream.isLoggedIn() && stream.isStreamingSupported())
                {
                    this.mc.displayGuiScreen(new GuiStreamOptions(this, this.game_settings_1));
                }
                else
                {
                    GuiStreamUnavailable.show(this);
                }
            }
        }
    }
