...
public class GuiEditSign extends GuiScreen
{
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException
    {
        ...
        {
            s = this.appendTextToLine(s, GuiScreen.getClipboardString());
        }
        else if (ChatAllowedCharacters.isAllowedCharacter(typedChar))
        {
            s = this.appendTextToLine(s, Character.toString(typedChar));
        }
    ...
    private String appendTextToLine(String line, String text)
    {
        String filteredText = ChatAllowedCharacters.filterAllowedCharacters(text);

        for (int i = 0; i < filteredText.length();)
        {
            int codePoint = filteredText.codePointAt(i);
            String candidateLine = line + new String(Character.toChars(codePoint));

            if (this.fontRendererObj.getStringWidth(candidateLine) > 90)
            {
                break;
            }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        {
            float signRotation = (float)(this.tileSign.getBlockMetadata() * 360) / 16.0F;
            GlStateManager.rotate(signRotation, 0.0F, 1.0F, 0.0F);
            GlStateManager.translate(0.0F, -1.0625F, 0.0F);
        }
        else
        {
            int i = this.tileSign.getBlockMetadata();
            float wallSignRotation = 0.0F;

            if (i == 2)
            {
                wallSignRotation = 180.0F;
            }

            if (i == 4)
            {
                wallSignRotation = 90.0F;
            }
            ...
            GlStateManager.rotate(wallSignRotation, 0.0F, 1.0F, 0.0F);
            GlStateManager.translate(0.0F, -1.0625F, 0.0F);
        }
