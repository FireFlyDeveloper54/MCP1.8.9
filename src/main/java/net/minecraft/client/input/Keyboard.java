...
public final class Keyboard
{
    ...
    static
    {
        ...
        try
        {
            for (int i = 0; i < fields.length; ++i)
            {
                ...
                if (Modifier.isStatic(field.getModifiers()) && Modifier.isPublic(field.getModifiers()) && Modifier.isFinal(field.getModifiers()) && field.getType() == int.class && field.getName().startsWith("KEY_") && !field.getName().endsWith("WIN"))
                {
                    int key = field.getInt(null);
                    if (key >= 0 && key < KEY_NAMES.length)
                    {
                        KEY_NAMES[key] = field.getName().substring(4);
                    }
                }
            }
        }
        ...
        map(GLFW.GLFW_KEY_MENU, KEY_RMENU);
        map(GLFW.GLFW_KEY_WORLD_1, KEY_CIRCUMFLEX);
    }
    ...
    public static int toLwjglKey(int glfwKey)
    {
        if (glfwKey < 0 || glfwKey >= GLFW_TO_LWJGL.length)
        {
            return KEY_NONE;
