...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private long unusedCompatibilityTimestamp = 0L;
    private final Thread mcThread = Thread.currentThread();
    private ModelManager modelManager;
    private BlockRendererDispatcher blockRenderDispatcher;
    volatile boolean running = true;
    public String debug = "";
    public boolean unusedCompatibilityFlagA = false;
    public boolean unusedCompatibilityFlagB = false;
    public boolean unusedCompatibilityFlagC = false;
    ...
    public void run()
    {
        ...
        while (true)
        {
            try
            {
                while (this.running)
                {
                    if (!this.hasCrashed || this.crashReporter == null)
                    {
                        ...
                        catch (OutOfMemoryError caughtOutOfMemoryError)
                        {
                            this.freeMemory();
            ...
            catch (MinecraftError caughtMinecraftError)
            {
                break;
            }
    ...
    private void startGame() throws IOException
    {
        ...
        logger.info("LWJGL Version: " + GlUtil.getVersion());
        this.setWindowIcon();
        ...
        try
        {
            GameWindow.setVsync(this.gameSettings.enableVsync);
        }
        catch (RuntimeException caughtOpenGLException)
        {
    ...
    private void createDisplay() 
    {
        ...
        GameWindow.setTitle("Minecraft 1.8.9");

        try
        {
            GameWindow.create(this.displayWidth, this.displayHeight, "Minecraft 1.8.9", 24, 0);
        }
    ...
    private void setInitialDisplayMode() 
    {
        ...
        {
            GameWindow.setVideoMode(new GameWindow.VideoMode(this.displayWidth, this.displayHeight));
        }
    }
    ...
    private void setWindowIcon()
    {
        ...
        if (util$enumos != Util.EnumOS.OSX)
        {
            ...
            try
            {
                ...
                {
                    GameWindow.setIcon(new ByteBuffer[] {this.readImageToBuffer(inputstream), this.readImageToBuffer(inputstream1)});
                }
            }
    ...
    {
        Set<GameWindow.VideoMode> set = Sets.<GameWindow.VideoMode>newHashSet();
        Collections.addAll(set, GameWindow.getAvailableModes());
        GameWindow.VideoMode displaymode = GameWindow.getDesktopMode();

        if (!set.contains(displaymode) && Util.getOSType() == Util.EnumOS.OSX)
        {
            label53:

            for (GameWindow.VideoMode displaymode1 : macDisplayModes)
            {
                boolean flag = true;

                for (GameWindow.VideoMode displaymode2 : set)
                {
                ...
                if (!flag)
                {
                    ...
                    GameWindow.VideoMode displaymode3;
                    ...
                    while (true)
                    {
                        ...
                        displaymode3 = (GameWindow.VideoMode)iterator.next();
    ...
    public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue, int alpha)
    {
        ...
        worldRenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldRenderer.pos((double)posX, (double)(posY + height), 0.0D).tex((double)((float)texU * f), (double)((float)(texV + height) * floatValue2)).color(red, green, blue, alpha).endVertex();
        worldRenderer.pos((double)(posX + width), (double)(posY + height), 0.0D).tex((double)((float)(texU + width) * f), (double)((float)(texV + height) * floatValue2)).color(red, green, blue, alpha).endVertex();
        worldRenderer.pos((double)(posX + width), (double)posY, 0.0D).tex((double)((float)(texU + width) * f), (double)((float)texV * floatValue2)).color(red, green, blue, alpha).endVertex();
        worldRenderer.pos((double)posX, (double)posY, 0.0D).tex((double)((float)texU * f), (double)((float)texV * floatValue2)).color(red, green, blue, alpha).endVertex();
    ...
    private void checkGLError(String message)
    {
        if (this.enableGLErrorChecking)
        {
            ...
            if (i != 0)
            {
                String s = GlUtil.gluErrorString(i);
    ...
    private void runGameLoop() throws IOException
    {
        ...
        if (GameWindow.isCreated() && GameWindow.shouldClose())
        {
            this.shutdown();
        }
        ...
        long secondLongValue = System.nanoTime() - l;
        ...
        if (this.gameSettings.showDebugInfo && this.gameSettings.showDebugProfilerChart && !this.gameSettings.hideGUI)
        {
            ...
            this.displayDebugInfo(secondLongValue);
        }
        else
        {
        ...
        while (getSystemTime() >= this.debugUpdateTime + 1000L)
        {
            ...
            Object framerateLimit = (float)this.gameSettings.limitFramerate == GameSettings.Options.FRAMERATE_LIMIT.getValueMax() ? "inf" : Integer.valueOf(this.gameSettings.limitFramerate);
            this.debug = debugFPS + " fps (" + RenderChunk.renderChunksUpdated + " chunk update" + (RenderChunk.renderChunksUpdated != 1 ? "s" : "") + ") T: " + framerateLimit + (this.gameSettings.enableVsync ? " vsync" : "") + (this.gameSettings.fancyGraphics ? "" : " fast") + (this.gameSettings.clouds == 0 ? "" : (this.gameSettings.clouds == 1 ? " fast-clouds" : " fancy-clouds")) + (OpenGlHelper.useVbo() ? " vbo" : "");
        ...
        if (this.isFramerateLimitBelowMax())
        {
            ...
            GameWindow.sync(this.getLimitFramerate());
            this.mcProfiler.endSection();
        }
    ...
    private void displayDebugInfo(long elapsedTicksTime)
    {
        if (this.mcProfiler.profilingEnabled)
        {
            ...
            for (int l = 0; l < list.size(); ++l)
            {
                ...
                int eighthIntValue = MathHelper.floor_double(profiler$result1.usePercentage / 4.0D) + 1;
                worldrenderer.begin(6, DefaultVertexFormats.POSITION_COLOR);
                int intValue = profiler$result1.getColor();
                int secondIntValue = intValue >> 16 & 255;
                int thirdIntValue = intValue >> 8 & 255;
                int fourthIntValue = intValue & 255;
                worldrenderer.pos((double)j, (double)k, 0.0D).color(secondIntValue, thirdIntValue, fourthIntValue, 255).endVertex();

                for (int fifthIntValue = eighthIntValue; fifthIntValue >= 0; --fifthIntValue)
                {
                    float f = (float)((doubleValue + profiler$result1.usePercentage * (double)fifthIntValue / (double)eighthIntValue) * Math.PI * 2.0D / 100.0D);
                    float floatValue = MathHelper.sin(f) * (float)i;
                    float secondFloatValue = MathHelper.cos(f) * (float)i * 0.5F;
                    worldrenderer.pos((double)((float)j + floatValue), (double)((float)k - secondFloatValue), 0.0D).color(secondIntValue, thirdIntValue, fourthIntValue, 255).endVertex();
                }

                tessellator.draw();
                worldrenderer.begin(5, DefaultVertexFormats.POSITION_COLOR);

                for (int sixthIntValue = eighthIntValue; sixthIntValue >= 0; --sixthIntValue)
                {
                    float thirdFloatValue = (float)((doubleValue + profiler$result1.usePercentage * (double)sixthIntValue / (double)eighthIntValue) * Math.PI * 2.0D / 100.0D);
                    float fourthFloatValue = MathHelper.sin(thirdFloatValue) * (float)i;
                    float fifthFloatValue = MathHelper.cos(thirdFloatValue) * (float)i * 0.5F;
                    worldrenderer.pos((double)((float)j + fourthFloatValue), (double)((float)k - fifthFloatValue), 0.0D).color(secondIntValue >> 1, thirdIntValue >> 1, fourthIntValue >> 1, 255).endVertex();
                    worldrenderer.pos((double)((float)j + fourthFloatValue), (double)((float)k - fifthFloatValue + 10.0F), 0.0D).color(secondIntValue >> 1, thirdIntValue >> 1, fourthIntValue >> 1, 255).endVertex();
                }

                tessellator.draw();
                doubleValue += profiler$result1.usePercentage;
            }
            ...
            int tenthIntValue = 16777215;
            this.fontRendererObj.drawStringWithShadow(s, (float)(j - i), (float)(k - i / 2 - 16), tenthIntValue);
            this.fontRendererObj.drawStringWithShadow(s = decimalformat.format(profiler$result.totalUsePercentage) + "%", (float)(j + i - this.fontRendererObj.getStringWidth(s)), (float)(k - i / 2 - 16), tenthIntValue);

            for (int seventhIntValue = 0; seventhIntValue < list.size(); ++seventhIntValue)
            {
                Profiler.Result profiler$result2 = (Profiler.Result)list.get(seventhIntValue);
                String stringValue = "";
                ...
                {
                    stringValue = stringValue + "[" + (seventhIntValue + 1) + "] ";
                }

                stringValue = stringValue + profiler$result2.profilerName;
                this.fontRendererObj.drawStringWithShadow(stringValue, (float)(j - i), (float)(k + i / 2 + seventhIntValue * 8 + 20), profiler$result2.getColor());
                this.fontRendererObj.drawStringWithShadow(stringValue = decimalformat.format(profiler$result2.usePercentage) + "%", (float)(j + i - 50 - this.fontRendererObj.getStringWidth(stringValue)), (float)(k + i / 2 + seventhIntValue * 8 + 20), profiler$result2.getColor());
                this.fontRendererObj.drawStringWithShadow(stringValue = decimalformat.format(profiler$result2.totalUsePercentage) + "%", (float)(j + i - this.fontRendererObj.getStringWidth(stringValue)), (float)(k + i / 2 + seventhIntValue * 8 + 20), profiler$result2.getColor());
            }
        }
    }
    ...
    public void toggleFullscreen()
    {
        try
        {
            ...
            GameWindow.setFullscreen(this.fullscreen);
            this.displayWidth = Math.max(1, GameWindow.getWidth());
            this.displayHeight = Math.max(1, GameWindow.getHeight());

            if (!this.fullscreen)
            {
                this.tempDisplayWidth = this.displayWidth;
                this.tempDisplayHeight = this.displayHeight;
            }
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            {
                int i = GameWindow.getEventButton();
                KeyBinding.setKeyBindState(i - 100, GameWindow.getEventButtonState());

                if (GameWindow.getEventButtonState())
                {
                    if (this.thePlayer.isSpectator() && i == 2)
                    {
                        this.ingameGUI.getSpectatorGui().selectCurrentItem();
                    }
                    else
                    {
                ...
                long longValue = getSystemTime() - this.systemTime;

                if (longValue <= 200L)
                {
                    int j = GameWindow.getEventDWheel();
                    ...
                    if (j != 0)
                    {
                        if (this.thePlayer.isSpectator())
                        {
                            ...
                            {
                                this.ingameGUI.getSpectatorGui().cycleSlot(-j);
                            }
                            else
                            {
                    ...
                    if (this.currentScreen == null)
                    {
                        if (!this.inGameHasFocus && GameWindow.getEventButtonState())
                        {
                            this.setIngameFocus();
                        }
                    }
            ...
            while (GameWindow.nextKeyEvent())
            {
                ...
                if (k != 0)
                {
                    KeyBinding.setKeyBindState(k, keyState);
                ...
                if (this.debugCrashKeyPressTime > 0L)
                {
                    ...
                    if (!GameWindow.isKeyDown(46) || !GameWindow.isKeyDown(61))
                    {
                        this.debugCrashKeyPressTime = -1L;
                    }
                }
                else if (GameWindow.isKeyDown(46) && GameWindow.isKeyDown(61))
                {
                ...
                if (keyState)
                {
                    ...
                    else
                    {
                        ...
                        if (k == 32 && GameWindow.isKeyDown(61) && this.ingameGUI != null)
                        {
                            this.ingameGUI.getChatGUI().clearChatMessages();
                        }

                        if (k == 31 && GameWindow.isKeyDown(61))
                        {
                            this.refreshResources();
                        }

                        if (k == 20 && GameWindow.isKeyDown(61))
                        {
                            this.refreshResources();
                        }

                        if (k == 33 && GameWindow.isKeyDown(61))
                        {
                            this.gameSettings.setOptionValue(GameSettings.Options.RENDER_DISTANCE, GuiScreen.isShiftKeyDown() ? -1 : 1);
                        }

                        if (k == 30 && GameWindow.isKeyDown(61))
                        {
                            this.renderGlobal.loadRenderers();
                        }

                        if (k == 35 && GameWindow.isKeyDown(61))
                        {
                            this.gameSettings.advancedItemTooltips = !this.gameSettings.advancedItemTooltips;
                            this.gameSettings.saveOptions();
                        }

                        if (k == 48 && GameWindow.isKeyDown(61))
                        {
                            this.renderManager.setDebugBoundingBox(!this.renderManager.isDebugBoundingBox());
                        }

                        if (k == 25 && GameWindow.isKeyDown(61))
                        {
                    ...
                    if (this.gameSettings.showDebugInfo && this.gameSettings.showDebugProfilerChart)
                    {
                        ...
                        for (int ninthIntValue = 0; ninthIntValue < 9; ++ninthIntValue)
                        {
                            if (k == 2 + ninthIntValue)
                            {
                                this.updateDebugProfilerName(ninthIntValue + 1);
                            }
                        }
                    }
                }
            ...
            for (int l = 0; l < 9; ++l)
            {
                if (this.gameSettings.keyBindsHotbar[l].isPressed())
                {
                    if (this.thePlayer.isSpectator())
                    {
                        this.ingameGUI.getSpectatorGui().onHotbarSelected(l);
                    }
                    else
                    {
    ...
    public void addServerStatsToSnooper(PlayerUsageSnooper playerSnooper)
    {
        ...
        playerSnooper.addClientStat("display_frequency", Integer.valueOf(GameWindow.getVideoMode().getFrequency()));
    ...
    {
        return GameWindow.getTimeMillis();
    }
    ...
    {
        int i = GameWindow.getEventKey();

        if (i != 0 && !GameWindow.isRepeatEvent())
        {
