...
public abstract class Container
{
    ...
    public ItemStack slotClick(int slotId, int clickedButton, int mode, EntityPlayer playerIn)
    {
            ...
            else
            {
                ...
                if (slot7 != null)
                {
                    ...
                    if (itemstack9 == null)
                    {
                        ...
                        {
                            int seventhIntValue = clickedButton == 0 ? itemstack10.stackSize : 1;

                            if (seventhIntValue > slot7.getItemStackLimit(itemstack10))
                            {
                                seventhIntValue = slot7.getItemStackLimit(itemstack10);
                            }

                            if (itemstack10.stackSize >= seventhIntValue)
                            {
                                slot7.putStack(itemstack10.splitStack(seventhIntValue));
                            }
                    ...
                    else if (slot7.canTakeStack(playerIn))
                    {
                        if (itemstack10 == null)
                        {
                            int fifthIntValue = clickedButton == 0 ? itemstack9.stackSize : (itemstack9.stackSize + 1) / 2;
                            ItemStack itemstack12 = slot7.decrStackSize(fifthIntValue);
                        ...
                        else if (slot7.isItemValid(itemstack10))
                        {
                            ...
                            {
                                int secondIntValue = clickedButton == 0 ? itemstack10.stackSize : 1;

                                if (secondIntValue > slot7.getItemStackLimit(itemstack10) - itemstack9.stackSize)
                                {
                                    secondIntValue = slot7.getItemStackLimit(itemstack10) - itemstack9.stackSize;
                                }

                                if (secondIntValue > itemstack10.getMaxStackSize() - itemstack9.stackSize)
                                {
                                    secondIntValue = itemstack10.getMaxStackSize() - itemstack9.stackSize;
                                }

                                itemstack10.splitStack(secondIntValue);
                                ...
                                itemstack9.stackSize += secondIntValue;
                            }
                        ...
                        {
                            int eighthIntValue = itemstack9.stackSize;

                            if (eighthIntValue > 0 && eighthIntValue + itemstack10.stackSize <= itemstack10.getMaxStackSize())
                            {
                                itemstack10.stackSize += eighthIntValue;
                                itemstack9 = slot7.decrStackSize(eighthIntValue);
        ...
        else if (mode == 2 && clickedButton >= 0 && clickedButton < 9)
        {
            ...
            if (slot5.canTakeStack(playerIn))
            {
                ...
                if (!flag)
                {
                    sixthIntValue = inventoryplayer.getFirstEmptyStack();
                    flag |= sixthIntValue > -1;
                }
        ...
        else if (mode == 6 && slotId >= 0)
        {
            ...
            {
                int intValue = clickedButton == 0 ? 0 : this.inventorySlots.size() - 1;
                int fourthIntValue = clickedButton == 0 ? 1 : -1;

                for (int ninthIntValue = 0; ninthIntValue < 2; ++ninthIntValue)
                {
                    for (int thirdIntValue = intValue; thirdIntValue >= 0 && thirdIntValue < this.inventorySlots.size() && itemstack4.stackSize < itemstack4.getMaxStackSize(); thirdIntValue += fourthIntValue)
                    {
                        Slot slot8 = this.inventorySlots.get(thirdIntValue);

                        if (slot8.getHasStack() && canAddItemToSlot(slot8, itemstack4, true) && slot8.canTakeStack(playerIn) && this.canMergeSlot(itemstack4, slot8) && (ninthIntValue != 0 || slot8.getStack().stackSize != slot8.getStack().getMaxStackSize()))
                        {
    ...
    public static int getDragEvent(int eventButton)
    {
        return eventButton & 3;
    }

    public static int combineDragData(int dragMode, int dragEvent)
    {
        return dragMode & 3 | (dragEvent & 3) << 2;
    }
    ...
    public static void computeStackSize(Set<Slot> dragSlots, int dragMode, ItemStack stack, int slotStackSize)
    {
        switch (dragMode)
        {
            case 0:
                stack.stackSize = MathHelper.floor_float((float)stack.stackSize / (float)dragSlots.size());
                break;
