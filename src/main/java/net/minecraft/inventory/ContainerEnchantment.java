...
public class ContainerEnchantment extends Container
{
        ...
        if (inventoryIn == this.tableInventory)
        {
            ...
            if (itemStack != null && itemStack.isItemEnchantable())
            {
                if (!this.worldPointer.isRemote)
                {
                    ...
                    for (int enchantmentSlot = 0; enchantmentSlot < 3; ++enchantmentSlot)
                    {
                        this.enchantLevels[enchantmentSlot] = EnchantmentHelper.calcItemStackEnchantability(this.rand, enchantmentSlot, l, itemStack);
                        this.enchantmentIds[enchantmentSlot] = -1;

                        if (this.enchantLevels[enchantmentSlot] < enchantmentSlot + 1)
                        {
                            this.enchantLevels[enchantmentSlot] = 0;
                        }
                    }

                    for (int enchantmentSlot = 0; enchantmentSlot < 3; ++enchantmentSlot)
                    {
                        if (this.enchantLevels[enchantmentSlot] > 0)
                        {
                            List<EnchantmentData> list = this.getPossibleEnchantments(itemStack, enchantmentSlot, this.enchantLevels[enchantmentSlot]);

                            if (list != null && !list.isEmpty())
                            {
                                EnchantmentData enchantmentData = (EnchantmentData)list.get(this.rand.nextInt(list.size()));
                                this.enchantmentIds[enchantmentSlot] = enchantmentData.enchantmentobj.effectId | enchantmentData.enchantmentLevel << 8;
                            }
                        }
                    }
    ...
    public boolean enchantItem(EntityPlayer playerIn, int id)
    {
        ...
        else if (this.enchantLevels[id] > 0 && itemStack != null && (playerIn.experienceLevel >= i && playerIn.experienceLevel >= this.enchantLevels[id] || playerIn.capabilities.isCreativeMode))
        {
            if (!this.worldPointer.isRemote)
            {
                List<EnchantmentData> list = this.getPossibleEnchantments(itemStack, id, this.enchantLevels[id]);
                ...
                if (list != null)
                {
                    ...
                    playerIn.triggerAchievement(StatList.itemEnchantedStat);
    ...
    private List<EnchantmentData> getPossibleEnchantments(ItemStack stack, int enchantmentSlot, int enchantability)
    {
        this.rand.setSeed((long)(this.xpSeed + enchantmentSlot));
        List<EnchantmentData> list = EnchantmentHelper.buildEnchantmentList(this.rand, stack, enchantability);
