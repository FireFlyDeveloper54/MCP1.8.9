...
public class StrUtils
{
    public static boolean equalsMask(String str, String mask, char wildChar, char wildCharSingle)
    {
        if (mask != null && str != null)
        {
            ...
            else
            {
                ...
                String wildcardToken = "" + wildChar;

                if (mask.startsWith(wildcardToken))
                {
                    segments.add("");
                }

                StringTokenizer tokenizer = new StringTokenizer(mask, wildcardToken);
                ...
                if (mask.endsWith(wildcardToken))
                {
                    segments.add("");
                }
                ...
                if (!startsWithMaskSingle(str, firstSegment, wildCharSingle))
                {
                    return false;
                }
                else
                {
                    ...
                    if (!endsWithMaskSingle(str, lastSegment, wildCharSingle))
                    {
                        return false;
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < ((List)segments).size(); ++i)
                        {
                            ...
                            if (segment.length() > 0)
                            {
                                int matchIndex = indexOfMaskSingle(str, segment, searchIndex, wildCharSingle);
        ...
        if (str != null && mask != null)
        {
            ...
            else
            {
                for (int i = 0; i < mask.length(); ++i)
                {
                    ...
                    if (character != wildCharSingle && str.charAt(i) != character)
                    {
                        return false;
                    }
                }
    ...
    private static boolean endsWithMaskSingle(String str, String mask, char wildCharSingle)
    {
        if (str != null && mask != null)
        {
            ...
            {
                String suffix = str.substring(str.length() - mask.length(), str.length());
                return equalsMaskSingle(suffix, mask, wildCharSingle);
            }
        }
        else
        {
    ...
    private static boolean startsWithMaskSingle(String str, String mask, char wildCharSingle)
    {
        if (str != null && mask != null)
        {
            ...
            {
                String prefix = str.substring(0, mask.length());
                return equalsMaskSingle(prefix, mask, wildCharSingle);
            }
        }
        else
        {
    ...
    public static boolean equalsMask(String str, String mask, char wildChar)
    {
        if (mask != null && str != null)
        {
            ...
            else
            {
                ...
                if (!str.startsWith(firstSegment))
                {
                    return false;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        for (int index = 0; index < ((List)segments).size(); ++index)
                        {
                            String segment = (String)segments.get(index);

                            if (segment.length() > 0)
                            {
                                int matchIndex = str.indexOf(segment, searchIndex);
        ...
        if (str != null && str.length() > 0)
        {
            ...
            else
            {
                ...
                for (int index = 0; index < str.length(); ++index)
                {
                    ...
                    if (equals(currentChar, separators))
                    {
                        parts.add(str.substring(startIndex, index));
                        startIndex = index + 1;
                    }
                }

                parts.add(str.substring(startIndex, str.length()));
    ...
    {
        int parsedValue = parseInt(str, -1);
        ...
        {
            ++parsedValue;
            String incremented = "" + parsedValue;
            return incremented.length() > str.length() ? "" : fillLeft("" + parsedValue, str.length(), '0');
        }
    }
    ...
    public static String addIfNotContains(String target, String source)
    {
        ...
        {
            if (target.indexOf(source.charAt(index)) < 0)
            {
                target = target + source.charAt(index);
            }
        }
