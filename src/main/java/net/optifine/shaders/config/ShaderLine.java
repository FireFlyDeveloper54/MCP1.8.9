...
public class ShaderLine
{
    ...
    public Vector4f getValueVec4()
    {
        ...
        {
            String vecText = this.value.trim();
            vecText = StrUtils.removePrefix(vecText, "vec4");
            vecText = StrUtils.trim(vecText, " ()");
            String[] componentTexts = Config.tokenize(vecText, ", ");

            if (componentTexts.length != 4)
            {
                return null;
            }
            else
            {
                ...
                for (int componentIndex = 0; componentIndex < componentTexts.length; ++componentIndex)
                {
                    String componentText = componentTexts[componentIndex];
                    componentText = StrUtils.removeSuffix(componentText, new String[] {"F", "f"});
                    float componentValue = Config.parseFloat(componentText, Float.MAX_VALUE);

                    if (componentValue == Float.MAX_VALUE)
                    {
                        return null;
                    }
    ...
    {
        String lowerValue = this.value.toLowerCase();
