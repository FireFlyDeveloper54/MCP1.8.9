...
public class Property
{
    ...
    {
        int minIndex = 0;
        int maxIndex = this.propertyValues.length - 1;
        this.value = Config.limit(this.value, minIndex, maxIndex);

        if (forward)
        {
            ++this.value;

            if (this.value > maxIndex)
            {
                this.value = minIndex;
            }
        }
    ...
    public boolean loadFrom(Properties props)
    {
        ...
        {
            String propertyValue = props.getProperty(this.propertyName);
            return propertyValue == null ? false : this.setPropertyValue(propertyValue);
        }
    }
