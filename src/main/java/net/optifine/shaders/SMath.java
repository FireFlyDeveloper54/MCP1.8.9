...
public class SMath
{
    static void multiplyMat4xMat4(float[] matOut, float[] matA, float[] matB)
    {
        for (int row = 0; row < 4; ++row)
        {
            ...
            {
                matOut[4 * row + column] = matA[4 * row + 0] * matB[0 + column] + matA[4 * row + 1] * matB[4 + column] + matA[4 * row + 2] * matB[8 + column] + matA[4 * row + 3] * matB[12 + column];
            }
        }
    }
    ...
    static void invertMat4(float[] matOut, float[] m)
    {
        ...
        float determinant = m[0] * matOut[0] + m[1] * matOut[4] + m[2] * matOut[8] + m[3] * matOut[12];

        if ((double)determinant != 0.0D)
        {
            for (int matrixIndex = 0; matrixIndex < 16; ++matrixIndex)
            {
                matOut[matrixIndex] /= determinant;
            }
        }
        else
        {
