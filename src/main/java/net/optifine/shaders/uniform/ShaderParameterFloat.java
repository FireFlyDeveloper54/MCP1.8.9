...
public enum ShaderParameterFloat
{
    ...
    static final ShaderParameterFloat[] VALUES = values();
    ...
    public float eval(int index1, int index2)
    {
        if (this.indexNames1 == null || index1 >= 0 && index1 <= this.indexNames1.length)
        {
            if (this.indexNames2 == null || index2 >= 0 && index2 <= this.indexNames2.length)
            {
                ...
                {
                    case BIOME:
                        BlockPos biomePosition = Shaders.getCameraPosition();
                        BiomeGenBase currentBiome = Shaders.getCurrentWorld().getBiomeGenForCoords(biomePosition);
                        return (float)currentBiome.biomeID;

                    case TEMPERATURE:
                        BlockPos temperaturePosition = Shaders.getCameraPosition();
                        BiomeGenBase temperatureBiome = Shaders.getCurrentWorld().getBiomeGenForCoords(temperaturePosition);
                        return temperatureBiome != null ? temperatureBiome.getFloatTemperature(temperaturePosition) : 0.0F;
    ...
    private static boolean instanceOf(Object obj, Class... classes)
    {
        ...
        else
        {
            ...
            for (int classIndex = 0; classIndex < classes.length; ++classIndex)
            {
                ...
                if (candidateClass.isAssignableFrom(oclass))
                {
                    return true;
                }
            }
