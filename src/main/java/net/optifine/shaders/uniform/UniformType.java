...
public enum UniformType
{
    ...
    private void updateUniformFloat3(IExpressionFloatArray expression, ShaderUniform3f uniform)
    {
        ...
        if (values.length != 3)
        {
            throw new RuntimeException("Value length is not 3, length: " + values.length);
        }
        else
        {
            uniform.setValue(values[0], values[1], values[2]);
        }
    }
    ...
    private void updateUniformFloat4(IExpressionFloatArray expression, ShaderUniform4f uniform)
    {
        ...
        if (values.length != 4)
        {
            throw new RuntimeException("Value length is not 4, length: " + values.length);
        }
        else
        {
            uniform.setValue(values[0], values[1], values[2], values[3]);
        }
    }
    ...
    public static UniformType parse(String type)
    {
        ...
        for (int typeIndex = 0; typeIndex < uniformTypes.length; ++typeIndex)
        {
            UniformType uniformType = uniformTypes[typeIndex];
