...
public class IteratorAxis implements Iterator<BlockPos>
{
    ...
    {
        BlockPos startPos = new BlockPos(-2, 10, 20);
        BlockPos endPos = new BlockPos(2, 12, 22);
        double yDelta = -0.5D;
        double zDelta = 0.5D;
        IteratorAxis iteratorAxis = new IteratorAxis(startPos, endPos, yDelta, zDelta);
        System.out.println("Start: " + startPos + ", end: " + endPos + ", yDelta: " + yDelta + ", zDelta: " + zDelta);

        while (iteratorAxis.hasNext())
        {
            BlockPos nextPos = iteratorAxis.next();
