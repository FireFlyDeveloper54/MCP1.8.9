...
public class BetterGrass
{
    ...
    public static void update()
    {
        if (spritesLoaded)
        {
            ...
            if (grassMultilayer)
            {
                IBakedModel sideModel = BlockModelUtils.makeModelCube((TextureAtlasSprite)spriteGrassSide, -1);
                modelCubeGrass = BlockModelUtils.joinModelsCube(sideModel, modelCubeGrass);
            }
    ...
    private static void loadProperties(TextureMap textureMap)
    {
        ...
        spritesLoaded = true;
        String propertiesPath = "optifine/bettergrass.properties";
        ...
        try
        {
            ...
            {
                Config.dbg("BetterGrass: Parsing default configuration " + propertiesPath);
            }
            else
            {
                Config.dbg("BetterGrass: Parsing configuration " + propertiesPath);
            }
        ...
        {
            Config.warn("Error reading: " + propertiesPath + ", " + ioException.getClass().getName() + ": " + ioException.getMessage());
        }
    }

    private static TextureAtlasSprite registerSprite(Properties props, String key, String textureDefault, TextureMap textureMap)
    {
        String texturePath = props.getProperty(key);

        if (texturePath == null)
        {
            texturePath = textureDefault;
        }

        ResourceLocation textureFileLocation = new ResourceLocation("textures/" + texturePath + ".png");

        if (!Config.hasResource(textureFileLocation))
        {
            Config.warn("BetterGrass texture not found: " + textureFileLocation);
            texturePath = textureDefault;
        }

        ResourceLocation textureLocation = new ResourceLocation(texturePath);
        TextureAtlasSprite textureSprite = textureMap.registerSprite(textureLocation);
    ...
    private static List getFaceQuadsMycelium(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, EnumFacing facing, List quads)
    {
        ...
        boolean hasSnowAbove = block == Blocks.snow || block == Blocks.snow_layer;
    ...
    private static List getFaceQuadsDirt(IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, EnumFacing facing, List quads)
    {
        ...
        else
        {
            ...
            if (Config.isBetterGrassFancy())
            {
                ...
                {
                    BlockPos adjacentPodzolPos = blockPos.down().offset(facing);
                    IBlockState adjacentPodzolState = blockAccess.getBlockState(adjacentPodzolPos);

                    if (adjacentPodzolState.getBlock() == Blocks.dirt && adjacentPodzolState.getValue(BlockDirt.VARIANT) == BlockDirt.DirtType.PODZOL)
                    {
    ...
    {
        BlockPos adjacentPos = blockPos.offset(facing);
        Block block = blockAccess.getBlockState(adjacentPos).getBlock();
        return block;
    }
