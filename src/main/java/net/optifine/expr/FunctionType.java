...
public enum FunctionType
{
    ...
    public float evalFloat(IExpression[] args)
    {
        switch (this)
        {
            ...
            case MOD:
                float modDividend = evalFloat(args, 0);
                float modDivisor = evalFloat(args, 1);
                return modDividend - modDivisor * (float)((int)(modDividend / modDivisor));
            ...
            case FMOD:
                float floorModDividend = evalFloat(args, 0);
                float floorModDivisor = evalFloat(args, 1);
                return floorModDividend - floorModDivisor * (float)MathHelper.floor_float(floorModDividend / floorModDivisor);
            ...
            case IF:
                int conditionCount = (args.length - 1) / 2;

                for (int conditionIndex = 0; conditionIndex < conditionCount; ++conditionIndex)
                {
                    int argumentIndex = conditionIndex * 2;

                    if (evalBool(args, argumentIndex))
                    {
                        return evalFloat(args, argumentIndex + 1);
                    }
                }

                return evalFloat(args, conditionCount * 2);

            case SMOOTH:
                int smoothId = (int)evalFloat(args, 0);
                float rawValue = evalFloat(args, 1);
                float fadeUp = args.length > 2 ? evalFloat(args, 2) : 1.0F;
                float fadeDown = args.length > 3 ? evalFloat(args, 3) : fadeUp;
                float smoothedValue = Smoother.getSmoothValue(smoothId, rawValue, fadeUp, fadeDown);
    ...
    private float getMin(IExpression[] exprs)
    {
        ...
        {
            float minValue = evalFloat(exprs, 0);

            for (int exprIndex = 1; exprIndex < exprs.length; ++exprIndex)
            {
                float value = evalFloat(exprs, exprIndex);
    ...
    private float getMax(IExpression[] exprs)
    {
        ...
        {
            float maxValue = evalFloat(exprs, 0);
    ...
    {
        IExpressionFloat expressionFloat = (IExpressionFloat)exprs[index];
        float value = expressionFloat.eval();
        return value;
    }
    ...
    public boolean evalBool(IExpression[] args)
    {
        switch (this)
        {
            ...
            case BETWEEN:
                float betweenValue = evalFloat(args, 0);
                return betweenValue >= evalFloat(args, 1) && betweenValue <= evalFloat(args, 2);

            case EQUALS:
                float equalsDelta = evalFloat(args, 0) - evalFloat(args, 1);
                float equalsTolerance = evalFloat(args, 2);
                return Math.abs(equalsDelta) <= equalsTolerance;

            case IN:
                float searchValue = evalFloat(args, 0);

                for (int argumentIndex = 1; argumentIndex < args.length; ++argumentIndex)
                {
                    float candidateValue = evalFloat(args, argumentIndex);
    ...
    {
        IExpressionBool expressionBool = (IExpressionBool)exprs[index];
        boolean value = expressionBool.eval();
        return value;
    }
    ...
    {
        for (int functionIndex = 0; functionIndex < VALUES.length; ++functionIndex)
        {
            FunctionType functionType = VALUES[functionIndex];
