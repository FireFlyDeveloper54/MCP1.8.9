package optimization.occlusionCulling;

import optimization.occlusionCulling.cache.ArrayOcclusionCache;
import optimization.occlusionCulling.cache.OcclusionCache;
import optimization.occlusionCulling.util.MathUtilities;
...
public class OcclusionCullingInstance {
    ...
    public boolean isAABBVisible(Vec3d aabbMin, Vec3d aabbMax, Vec3d viewerPosition) {
        try {
            int maxX = MathUtilities.fastFloor(aabbMax.x
                 + aabbExpansion);
            int maxY = MathUtilities.fastFloor(aabbMax.y
                 + aabbExpansion);
            int maxZ = MathUtilities.fastFloor(aabbMax.z
                 + aabbExpansion);
            ...
            cameraPos[0] = MathUtilities.fastFloor(viewerPosition.x);
            cameraPos[1] = MathUtilities.fastFloor(viewerPosition.y);
            cameraPos[2] = MathUtilities.fastFloor(viewerPosition.z);
    ...
    private boolean isVisible(Vec3d start, Vec3d[] targets, int size) {
        ...
        for (int v = 0; v < size; v++) {
            ...
            if (dimensionX == 0f) {
                ...
                // step by 1
                intersectCount += MathUtilities.fastFloor(target.x) - x; // increment total amount of intersecting cells
                ...
                // by 1
                intersectCount += x - MathUtilities.fastFloor(target.x); // increment total amount of intersecting cells
            ...
            if (dimensionY == 0f) {
                ...
                // step by 1
                intersectCount += MathUtilities.fastFloor(target.y) - y; // increment total amount of intersecting cells
                ...
                // by 1
                intersectCount += y - MathUtilities.fastFloor(target.y); // increment total amount of intersecting cells
            ...
            if (dimensionZ == 0f) {
                ...
                // step by 1
                intersectCount += MathUtilities.fastFloor(target.z) - z; // increment total amount of intersecting cells
                ...
                // by 1
                intersectCount += z - MathUtilities.fastFloor(target.z); // increment total amount of intersecting cells
    ...
    private void cacheResult(Vec3d vector, boolean result) {
        int cx = MathUtilities.fastFloor(vector.x) - cameraPos[0] + reach;
        int cy = MathUtilities.fastFloor(vector.y) - cameraPos[1] + reach;
        int cz = MathUtilities.fastFloor(vector.z) - cameraPos[2] + reach;
        if (result) {
            cache.setVisible(cx, cy, cz);
        } else {
            cache.setHidden(cx, cy, cz);
